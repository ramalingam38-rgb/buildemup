# MULTI-FLOOR DWELLING BRIEF SPEC v0.4 PROPOSED — domain dataclass for multi-floor residential briefs (B-NEW-T3 enabler #1 of 4)

**Component**: NEW domain dataclass (lives at `buildemup/domain/multi_floor_brief.py`).
**Spec status**: **v0.4 PROPOSED. PENDING Ramalingam LOCK adjudication.**
...
**Authored**: S40-continuation, post-v0.3 critique walk.
**Driver**: First of four locked specs needed before B-NEW-T3 can build.

**Note on v0.4 vs v0.3**: this is a documentation-polish round. **No schema changes. No new fields, helpers, invariants, or tests.** All v0.4 patches are wording/clarity/forward-compat-doc improvements applied during the v0.3 critique walk. Functionally and structurally identical to v0.3.
...
NEVER self-declares LOCK. This file is `v0.4 PROPOSED`. After Ramalingam
adjudication, the next file in `02_specs_chronological/` will be either:
- `..._v0_4_LOCKED.md` (if approved as-is),
- `..._v0_5_PROPOSED.md` (if patches surfaced — Rule 8 patch-eligible
  until LOCK).
...
## § 0.1 — Delta from v0.3 → v0.4 (critique walk patches)

v0.3 PROPOSED received an external critique with 13 items. The reviewer
explicitly noted "v0.3 is genuinely close to LOCK quality" and that
remaining issues are "mostly refinement-level rather than foundational
architecture defects." Verdicts:

| Item | Verdict | Patch site |
|---|---|---|
| CRITICAL-1 (nested rewrite affects equality/hash, not just identity) | ACCEPT | § 2 docstring; § 3.1 — explicit set-membership scenario |
| MAJOR-1 (`floor_has_master` semantics depend on normalization) | ACCEPT | § 3.3 + helper docstring |
| MAJOR-2 (identity vs display label conflation — future migration risk) | ACCEPT (doc note) | § 3.1 |
| MAJOR-3 (`iter_floors_with_master_flag` payload shape rigidity) | ACCEPT (doc note) | § 3.3 DESIGN NOTE |
| MAJOR-4 (MFDB-5 wording — "exactly one" overstated) | ACCEPT (rewording) | § 2 docstring; § 3.2 |
| MODERATE-1 (normalization-determinism contract too narrow — only mentions cache) | ACCEPT | § 3.6 — broaden from "cache protocol" to "identity protocol" |
| MODERATE-2 (auto-normalization hides caller bugs) | BACKLOG ONLY | New B-MFDB-M (strict-mode lookup helpers) |
| MODERATE-3 (spec relies heavily on prose contracts — maturity signal) | REJECT | None — meta-observation, not actionable |
| MODERATE-4 (no semantics for floor reordering) | ACCEPT | § 3.5 — forward-compat clause |
| MINOR-1 (embedded "v0.3" references age poorly) | PARTIAL ACCEPT | Targeted cleanup — historical critique-walk references kept; runtime-behavior references use "current version" |
| MINOR-2 (`_VALID_SELECTORS` tuple over-engineered for one value) | REJECT | None — forward-compat for negligible cost |
| MINOR-3 (normalization-in-domain is a permanent architectural choice) | REJECT | None — already openly acknowledged |
| MINOR-4 (test count growing) | REJECT | None — non-actionable maturity signal |

**Net schema delta from v0.3**: NONE. v0.4 adds zero new fields, helpers,
invariants, validations, or tests. All changes are documentation/wording.

Three rejects with reasoning preserved (MODERATE-3, MINOR-2, MINOR-3,
MINOR-4 — four total, MODERATE-3 and MINOR-4 are the same maturity-signal
class).

**Convergence signal**: this is the third critique round. v0.1 → v0.2 made
schema changes (new field, new helpers). v0.2 → v0.3 made schema changes
(runtime validation). v0.3 → v0.4 makes NO schema changes — the spec has
converged at the structural level. v0.4 is final-polish.
...
- 1 new file: `buildemup/domain/multi_floor_brief.py` (~190 lines incl.
  docstrings; ~205 lines after v0.4 documentation expansions).
...
- Patch-eligibility: critique surfaced before LOCK produces v0.5 PROPOSED.
- After LOCK: spec drafting proceeds to Spec #2 (C9 Amendment v0.8).
- Code work: deferred until all four specs LOCKED.
- **Convergence note**: v0.3 → v0.4 is the first round with NO schema
  changes (zero new fields, helpers, invariants, validations, or tests).
  All v0.4 patches are documentation refinements. Functional and
  structural shape has stabilized.
...
**End of MultiFloorDwellingBrief Spec v0.4 PROPOSED.**
