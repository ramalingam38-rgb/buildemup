# C11A SPEC AMENDMENT v1.4 PROPOSED — multi-floor pipeline rework (B-NEW-T3 enabler #4 of 4)
...
**Spec status**: **v1.4 PROPOSED. PENDING Ramalingam LOCK adjudication.**
**Authority**: Ramalingam directive at S40-continuation: spec-first, four-spec sequence for B-NEW-T3.
**Authored**: S40-continuation, post-v1.3 critique walk.
**Driver**: Spec #4 of 4, the largest. Completes the B-NEW-T3 spec sequence.

**v1.4 vs v1.3**: third consecutive behavioral-change round, addressing the explicit LOCK gate from v1.3 critique walk. **One real algorithmic correction** (full bipartite operator+floor interleaving replaces v1.3's operator-major-with-floor-rotation — eliminates operator-bias under slot truncation, which v1.3 fixed only the floor axis of). Plus structured `FloorImpact` return type on `affected_floor_set`, determinism tier-table, stateful mutation-chain property test, cross-spec integration tests, terminology pass for stale v1.1/v1.2 references, 3 new backlog items.
...
NEVER self-declares LOCK. This file is `v1.4 PROPOSED`. After Ramalingam
adjudication, the next file in `02_specs_chronological/` will be either:
- `..._v1_4_LOCKED.md` (if approved as-is),
- `..._v1_5_PROPOSED.md` (if patches surfaced — Rule 8 patch-eligible
  until LOCK).
...
This is a refinement chain, not a bandage chain. Each round replaces the previous algorithm with a mathematically-stronger one. Pattern-A check holds: v1.4's bipartite interleaving is a clean replacement of v1.3's operator-major loop, not a patch on top of it. Same complexity class (two nested loops), cleaner fairness property (both axes uniform under truncation).
...
Under truncation: M0 gets 2 attempts, M2 gets 2, M3 gets 1, M4 gets 1. All operators represented; floor distribution also remains uniform across the truncated set ({F0, F1, F2, F3} → each gets exactly 1 attempt from round 0, plus F1 and F2 from round 1).

**Fairness properties** under bipartite interleaving:
- **Per-operator slot count** under truncation at S: `floor(S / num_operators)` minimum to `ceil(S / num_operators)` maximum (off-by-one max).
- **Per-floor slot count** under truncation at S: same bounds via the operator-rotated offset.
- **Worst-case starvation**: bounded at `1 - 1/num_operators` of a slot per operator (vs v1.3's worst case where late operators could get zero).

**Single-floor briefs**: when `num_floors == 1`, the round loop iterates once and emits one attempt per operator. Existing single-floor behavior preserved byte-identical.

**Why this matters for evolutionary search**: under slot pressure, v1.3's operator-major order could systematically prevent certain mutation families from being explored, causing search convergence bias toward early-operator families' topology classes. v1.4's bipartite interleaving guarantees every operator gets representation proportional to the slot budget, which is the right property for evolutionary diversity.
...
- v1.4 (bipartite round-major interleaving) — both axes uniform under truncation.

Reviewer-named "LOCK gate" for v1.3 → v1.4 — addressed.

**Further refinements** (per-floor attempt weighting if uniform bipartite produces undesirable patterns; adjacency-aware floor prioritization; operator-priority weighting based on observed valid-result rates) filed as B-C11A-5.
...
### § 3.11 — `affected_floor_set(operator, source)` abstraction (NEW in v1.3 per item 15 from v1.2 critique walk; refined in v1.4 per item 6 from v1.3 critique walk)
...
def affected_floor_set(
    ...
    *,
    ...
    v1.4 contract:
      - Per-floor operators (M0-M7, M9): returns {FloorImpact(label,
        "direct", requires_cascade=True, requires_validation_only=False)}
        where label is the target_floor_label.
      - M8 (dwelling-wide): returns {
            FloorImpact(old_master, "direct", cascade=True, validation_only=False),
            FloorImpact(new_master, "direct", cascade=True, validation_only=False),
    ...
    a directly-mutated floor but whose own per-floor WZPC is reused:
      - M8 on a dwelling with vertical wet-stacks: indirect impacts on
        floors that share plumbing stacks with old/new master floors.
      - M7a/M7b on a dwelling with vertical column alignment: indirect
        impacts on floors that share load-bearing columns.
...
**Hardcoded "exactly two floors affected"** (current § 3.4 step 5) becomes a SPECIFIC CASE of the general contract: `affected_floor_set(M8, source, target_floor_label=new_master)` returns two `FloorImpact(kind="direct", requires_cascade=True)` entries. The orchestrator uses these to decide which floors need cascade re-runs vs which can be reused as-is. v1.4's implementation has exactly the same runtime behaviour as v1.3 (every affected floor has `requires_cascade=True`); v1.4's structured return type just makes the future-extension path additive.
...
**Consumers in v1.4**:
- § 3.4 step 5 (M8 cascade re-run): iterates the `FloorImpact` set, runs cascade on floors with `requires_cascade=True`, runs constraint validation on floors with `requires_validation_only=True` (no such floors today, but the loop is structured for them).
- § 3.2 / § 3.3 (per-floor operators): use `affected_floor_set(operator, source, target_floor_label=L)` to confirm exactly one `FloorImpact(L, "direct", cascade=True)` is returned before calling `with_floor_replaced(L, new_wzpc)`.

The abstraction is THIN — runtime behaviour unchanged from v1.3, future-amendment ergonomics significantly improved by the structured return type.
...
28. `test_b_new_t3_full_pipeline_3_floor_master_cycle`: construct 3-floor brief (master on ground); apply M8 across 3 distinct generations; verify cyclic target-selection guarantees coverage (each non-master floor becomes master exactly once); verify each intermediate wrapper passes MFWZP-5 (exactly one master); verify the final wrapper after 3 M8 applications has master on a deterministic-predictable floor per cyclic algorithm.
...
| **B-C11A-13** (NEW from v1.3 critique walk item 2) | Source-signature-stable-shuffle M8 target selection to break cyclic algorithm's evolutionary periodicity. v1.4's cyclic algorithm `sorted_targets[(generation + operator_index) % N]` guarantees coverage but creates deterministic periodicity (generations `g, g+N, g+2N, ...` all target the same floor). When C11b NSGA-II coordinates exploration pressure across population members, predictable target sequences may cause phase-locked exploration synchronizing across candidates. Upgrade path: `permutation = stable_shuffle(targets, source_signature); index = generation % N`. Preserves determinism + coverage; breaks per-candidate phase-locking | When C11b NSGA-II lands and exploration-pressure coordination becomes a real concern OR when empirical search runs show evolutionary resonance patterns | Open (post-v1) |
...
- **v1.4 PROPOSED. PENDING Ramalingam LOCK adjudication.**
- Authority: Rule 8 — LOCK authority belongs to Ramalingam alone.
- Patch-eligibility: critique surfaced before LOCK produces v1.5 PROPOSED.
- After LOCK: **B-NEW-T3 build can begin** (all four specs LOCKED).
- **Convergence note**: v1.1 → v1.2 → v1.3 → v1.4 is a three-round behavioral-change sequence on the operator-and-floor dispatch axis. Each round replaced the previous algorithm with a mathematically-stronger one — never bandages on top. v1.4 addresses the explicit LOCK gate from v1.3 critique walk. Reviewer's overall verdict ("v1.3 is plausibly LOCK-worthy IF... full interleaving is added before LOCK. Everything else is reasonable backlog territory") explicitly named bipartite interleaving as the single pre-LOCK blocker. v1.4 implements it. The remaining v1.3 critique items are documentation patches (§ 3.4.1 tier-table, § 3.11 FloorImpact, terminology pass, integration tests, stateful chain test) plus 3 new backlog items. Plausibly the final pre-LOCK round.
...
| B-C11A-13 | Source-signature-stable-shuffle M8 selection (break cyclic periodicity) | v1.4 critique | C11b coordinates exploration pressure | OUT-OF-SCOPE | S (post-v1) |
...
**Summary**: largest of the four B-NEW-T3 specs. Threads multi-floor through every C11a pipeline stage. v1.4 adds 3 new backlog items (B-C11A-13, 14, 15) on top of v1.3's 3 (B-C11A-10, 11, 12), v1.2's 5 (B-C11A-5 through 9), and v1.1's 4 (B-C11A-1 through 4) — 15 total. v1.4 also addresses the explicit pre-LOCK gate from v1.3 critique walk (bipartite operator+floor interleaving replaces operator-major round-robin). After LOCK, the B-NEW-T3 build session begins — the largest implementation work item in S41+.

---

**End of C11a Amendment v1.4 PROPOSED.**
