# C11b — Local Refinement (NSGA-II) — SPEC v0.7 PROPOSED

**Component**: 11b (canonical Track 3 numbering — paired with C11a Topology Mutation)
**Status**: **v0.7 PROPOSED. NOT LOCKED.** PENDING Ramalingam adjudication per Rule 8.
**Authored**: S41 close, post Walk #6 external critique on v0.6 PROPOSED.
**Supersedes**: v0.6 PROPOSED (carried forward verbatim except where this amendment marks changes).
**Predecessors**: v0.1 PROPOSED (S35), v0.2 PROPOSED (S36), v0.3 PROPOSED → v1.0 LOCKED (S37, SUSPENDED), v0.4 PROPOSED (S41), v0.5 PROPOSED (S41), v0.6 PROPOSED (S41).
...
- Authored at S41 close to address Walk #6 external critique on v0.6 PROPOSED.
- Walk #6: 15 critique items routed (Rule 7 + Rule 11 + 2 web searches on dominance hash structures + replay fingerprint versioning + code grep on `buildemup/utilities/canonical.py`):
  - 6 PATCH-NOW correctness/UX items (Items 1, 3, 4, 6, 8, 15) — folded into v0.7 body
  - 4 PATCH-NOW backlog-text amendments (Items 5, 9, 10, 13) — sharpened triggers, no new items
  - 1 NEW BACKLOG item (B-C11B-TIER2-SCALING, Item 2)
  - 4 ALREADY-DOCUMENTED items promoted to high priority (Items 7, 11, 12, 14)
- **Notable finding (HIGH severity, missed across 2 walks)**: Item 3 surfaced a stealth correctness bug — `tiebreak_fingerprint` was vulnerable to silent drift if `canonical_serialize` semantics ever changed without bumping `c11b_version`. v0.7 W6-3 closes the hole with explicit `TIEBREAK_FINGERPRINT_SCHEMA_VERSION` + `EnvironmentFingerprint.tiebreak_fingerprint_schema_version`. Rule 11 self-analysis should have caught this in v0.6; it didn't.
- 0 open questions remain.
- 1 audit finding carried from v0.4 (F-v4-7). v0.7 Rule 11 self-pass adds 6 findings — see § 9c below; 0 are PATCH-NOW blockers.
- 29 backlog items (12 carried + 2 v0.4 + 6 v0.5 + 8 v0.6 + 1 v0.7).
- 29 invariants (unchanged; Inv 29 wording continues from v0.6, + § 0.7.2 freezes TIER-3 minimum semantics).
- Subsystem count unchanged at 10 (cap held across all 6 walks).
- 38 new tests (v0.4 + v0.5 + v0.6 + v0.7) vs v1.0 LOCKED baseline.
- **Weighted complexity metric** (B-C11B-COMPLEXITY-BUDGET-V2 preview): 48 (4.8× the subsystem count of 10) — see § 0 narrative for breakdown.
...
**End of v0.7 PROPOSED.** Awaits Ramalingam adjudication: LOCK or Walk #7.
