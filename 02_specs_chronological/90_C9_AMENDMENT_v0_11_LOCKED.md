# C9 SPEC AMENDMENT v0.11 LOCKED — `has_master_bedroom` flag on `FloorRoomBrief` (B-NEW-T3 enabler #2 of 4)

**Component**: 9 (Room Sizing — SHIPPED at v0.7 LOCKED).
**Spec status**: **v0.11 LOCKED.** Ramalingam directive: "Do the patches and lock it and move in to the next spec doc" (S40-continuation).
**Authority**: Ramalingam directive at S40-continuation. LOCK granted at v0.11 after four critique rounds (v0.8 → v0.9 → v0.10 → v0.11).
...
**Note on this file vs file 89 (PROPOSED)**: file 89 preserves the PROPOSED text for chronological history per Rule 3. File 90 is the LOCKED version with status fields updated. Spec content (schema, invariants, behaviour, test plan) is identical between files.

---

## § 0 — LOCK declaration

**v0.11 LOCKED at S40-continuation per Ramalingam directive ("Do the patches and lock it and move in to the next spec doc").**

Per Rule 8 (LOCK authority belongs to Ramalingam alone), this declaration constitutes the LOCK trigger. v0.11 PROPOSED contents are promoted to v0.11 LOCKED with no further patches. The PROPOSED text is preserved chronologically at file 89 per Rule 3.

**Critique walk history**:
- v0.8 PROPOSED → critique walk #1 → v0.9 PROPOSED (CRITICAL-1 orchestration-contextual framing; MAJOR-3 trust boundary; MINOR-2 hash participation).
- v0.9 PROPOSED → critique walk #2 → v0.10 PROPOSED (no schema changes; precedent narrowing, lifecycle clarifier, "WILL own" softener, B-C9-D re-tag, reuse-safety clarifier).
- v0.10 PROPOSED → critique walk #3 → v0.11 PROPOSED (no schema changes; equality-overload acknowledgement, precedent worked examples, typo fix).
- v0.11 PROPOSED → **LOCKED**.

**Convergence summary**: schema stabilized at v0.8. Three consecutive critique rounds produced wording refinements only. The 1 new field (`has_master_bedroom: bool = True`), the 2 line-edits in `_materialise_rooms`, the 1 cache-version constant bump (`v1.0.0` → `v1.1.0`), and the test plan (~9 tests) are unchanged from v0.8 onward.
...
- **v0.11 LOCKED at S40-continuation per Ramalingam directive.**
- Authority: Rule 8 — LOCK authority belongs to Ramalingam alone.
- After LOCK: spec drafting proceeds to **Spec #3 (`MultiFloorWetZonePlannedCandidate`)** in this same session.
- Code work: deferred until all four specs LOCKED. Specs #3 and #4 still pending.

**Spec #2 of 4 — DONE.** Two remaining: `MultiFloorWetZonePlannedCandidate`, C11a Amendment v1.1.
...
**End of C9 Amendment v0.11 LOCKED.**

**Spec #2 of 4 in the B-NEW-T3 sequence — LOCKED.** Next: Spec #3 (`MultiFloorWetZonePlannedCandidate`).
