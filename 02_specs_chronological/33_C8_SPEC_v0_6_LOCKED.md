# BuildemUp — C8 Corridor Designer — SPEC v0.6 LOCKED

**Status**: **LOCKED at S33 close (5 May 2026) by Ramalingam.** Per Rule 8, LOCK authority belongs to Ramalingam alone.

**Predecessor**: v0.5 LOCKED (S31 close, S32 SHIP). Built and tested at S32 (174 tests passing); spec defect surfaced during build, filed as **B-129**. v0.6 PROPOSED resolves B-129 via internal mathematical reconciliation of § 4.3.1 truncation rule with Inv 20 invariant.

**Origin of v0.6 amendment**: B-129 — § 4.3.1 truncates `taper_zone_m ≤ length / 2` per side, but Inv 20 (text in v0.5) requires `taper_zone_m ≤ length / 4` per side under the parenthetical "assuming both ends taper". For one-end-taper segments the v0.5 rule is correct; for two-end-taper segments it permits middle = 0, violating Inv 20. v0.6 generalizes both to be N-aware where N = `n_tapered_ends ∈ {0, 1, 2}` (count of segment ends where width transitions away from `constant_width_m`).

**Original LOCK lineage** (preserved for chronology): v0.5 LOCKED by Ramalingam at S31 close, after walks #1-#4 + reviewer-confirmation walk #5. Built S32, SHIPPED S32. Post-SHIP defect → v0.6 PROPOSED.

**B-NNN concrete-number assignments (NEW v0.5 LOCKED)**: Placeholders A through R from v0.1-v0.5 PROPOSED iterations are assigned concrete IDs in this LOCKED file:
- B-NNN-A → **B-108** (NBC residential corridor minimum width primary-source verification)
- B-NNN-B → **B-109** (CorridorTooNarrowError graceful fallback)
- B-NNN-C → **B-111** (Rotated structural grid support)
- B-NNN-D → **B-112** (TNCDBR rule 42 + city-specific corridor overrides)
- B-NNN-E → **B-113** (Diagonal corridor segments)
- B-NNN-F → **B-114** (Multi-floor vertical corridor coordination)
- B-NNN-G → **B-115** (Variable-width within single segment)
- B-NNN-H → **B-116** (Per-room ZoneBandEnvelope refinement)
- B-NNN-I → **B-117** (Cross-segment width interaction model)
- B-NNN-J → **B-118** (Area-budget-aware corridor sizing)
- B-NNN-K → **B-119** (Band-importance-weighted strip allocation)
- B-NNN-L → **B-120** (Polygon-union via shapely fallback)
- B-NNN-M → **B-121** (Junction transition segments / fillets)
- B-NNN-N → **B-122** (Empirical thresholds for consumption_band)
- B-NNN-O → **B-123** (Width-selection scoring + taper-length empirical calibration)
- B-NNN-P → **B-124** (Width interpolation in junction-adjacent regions implementation)
- B-NNN-Q → **B-125** (Envelope-symmetry secondary-criterion weight calibration)
- B-NNN-R → **B-126** (Caller-side retry helper for CorridorDispatchError)
- B-110 → **B-110** (already concrete; C5 ↔ C8 length-drift integration test)

**Backlog after LOCK**: max = B-126 (was B-107 pre-LOCK; +19 items via this LOCK; backlog body in this spec § 12 plus mirror in `04_backlog/v0_2_backlog.md`).
...
**LOCK session**: S31 (post C6 SHIP, post v0.5 walk #5 confirmation).
**Predecessor**: v0.5 PROPOSED (S31; walk #4 addendum).

---

## § 0 — Lineage delta from v0.5 LOCKED → v0.6 PROPOSED (NEW S33)

**Drivers**: post-SHIP defect (B-129) surfaced during S32 C8 build. The v0.5 § 4.3.1 truncation rule and the v0.5 Inv 20 invariant are mutually inconsistent on segments with two tapered ends.

**The defect, restated precisely**:
- v0.5 § 4.3.1: `max_allowed = segment_length_m / 2.0` (comment: "one taper per end").
- v0.5 Inv 16: `2 × taper_zone_m ≤ length_m` (always; per-side bound regardless of N).
- v0.5 Inv 20 text: "`constant_middle_length_m ≥ length_m / 2`. … Combined with invariant 16, this means `taper_zone_m ≤ length_m / 4` per side (assuming both ends taper)."

The parenthetical "assuming both ends taper" reveals the inconsistency: when both ends *do* taper, Inv 20 needs `taper ≤ length / 4`, but § 4.3.1 still allows up to `length / 2`. With taper = length/2 on both ends, total taper = length, middle = 0 — Inv 20 violated. v0.5's `CorridorSegment.constant_middle_length_m = length - 2 × taper_zone_m` always assumed both ends taper, which is also wrong for the dominant case (single-junction BRANCH segments) — middle would be undercounted by `taper_zone_m` for one-end-taper segments.

**v0.6 resolution — N-aware truncation + N-aware middle**:

Define `n_tapered_ends ∈ {0, 1, 2}` for each segment as the count of segment ends where the actual width transitions from the segment's `constant_width_m`:
- N=0: uniform-width segment (no junction-induced taper).
- N=1: one end has a width transition (single-junction segment, dominant case for BRANCH).
- N=2: both ends have width transitions (e.g., interior PRIMARY segment between two junctions of different widths).

Updated rules:
- **§ 4.3.1**: `max_allowed = segment_length_m / (2.0 × max(1, n_tapered_ends))`. Caller passes `n_tapered_ends` derived from the segment's neighborhood.
- **Schema (`CorridorSegment.constant_middle_length_m`)**: `length_m - n_tapered_ends × taper_zone_m`. Property derives N from `start_width_m` / `end_width_m` vs `constant_width_m` (within `EPSILON_M`).
- **Inv 16**: text unchanged (`2 × taper_zone_m ≤ length_m`); becomes the binding bound only for N=1 (where it equals the new § 4.3.1 truncation). Subsumed by the new truncation for N=2.
- **Inv 20**: text updated — drop the "(assuming both ends taper)" parenthetical; restate as `constant_middle_length_m ≥ length_m / 2` where the formula for `constant_middle_length_m` now counts actual tapers.

**Math verification** (boundary cases hold for all N):

| N | max_allowed (§ 4.3.1) | total taper (worst case) | middle (worst case) | Inv 20 (`middle ≥ length/2`) |
|---|---|---|---|---|
| 0 | n/a (no taper) | 0 | length | ✓ |
| 1 | length / 2 | 1 × length/2 = length/2 | length/2 | ✓ (boundary) |
| 2 | length / 4 | 2 × length/4 = length/2 | length/2 | ✓ (boundary) |

**Note on the B-129 sketch**: the resolution path drafted in B-129's backlog entry suggested `length / (2 + 2 × N)`. That formula is incorrect (would give length/4 for N=1 and length/6 for N=2 — needlessly restrictive and would change the v0.5 § 4.10 worked example outcome). The correct formula is `length / (2N)`. v0.6 uses the correct form. B-129 backlog entry will be updated to point to v0.6 § 4.3.1 as the resolution.

**Implementation status of v0.6 amendment**:
- The S32 production code (`buildemup/components/c08/schema.py`) already implements the N-aware `constant_middle_length_m` formula (the deviation Δ.4 of master doc v3.7 documented). v0.6 codifies this in spec.
- The S32 production code's `width_selection.py` / `junction_propagation.py` callers of `resolve_taper_zone_m()` still pass the v0.5 single-arg signature. Code patches needed: thread `n_tapered_ends` through the call chain (small change, ~3 call sites). Filed as new code-side patch in § 12.

**Web research (Rule 7)**: confirmed at S33 open — NBC 2016 prescribes minimum corridor widths and width-vs-length escalation (`+0.15m per 3m additional length`), but no taper or transition-zone standards for residential corridors. v0.5's "no industry-standards guidance" stands. Search log preserved in S33 transcript.

**Spec changes vs v0.5 LOCKED**:
- Header banner (top of file) — Rule 8 PROPOSED status.
- New § 0 lineage delta (this section).
- § 3 schema text — `CorridorSegment.constant_middle_length_m` formula updated to N-aware.
- § 4.3.1 — `resolve_taper_zone_m()` signature gains `n_tapered_ends` parameter; `max_allowed` formula updated.
- § 4.10 worked example — N=1 case unchanged; new N=2 sub-table added for completeness.
- § 6 invariants — Inv 16 unchanged; Inv 20 text refined (parenthetical generalized).
- § 12 — B-129 marked RESOLVED (in-spec resolution; backlog entry retained but cross-references v0.6 § 4.3.1).

**No breaking changes** to public API (`design_corridors()` signature unchanged; `CorridorPath`/`CorridorSegment` field shapes unchanged; only internal property derivation refined).

---

## § 0.1 — Lineage delta from v0.4 PROPOSED → v0.5 PROPOSED (carried forward; v0.5-era heading kept for chronology)

**Drivers**: 6-item reviewer critique walk #4 + Ramalingam adjudication (option B: file B-NNN-R as proper C8 backlog item before LOCK rather than as post-LOCK project-backlog item).

**Walk #4 verdict summary**:
- 5 of 6 items: **DOCUMENTED — no action** (reviewer confirmed v0.4's handling is correct; items are acknowledgments, not new critiques)
- 1 of 6 items: **VALID-BUT-BACKLOG** (Drawback 3 → B-NNN-R)

| # | Walk #4 Drawback | PDF Status | Verdict | v0.5 action |
|---|---|---|---|---|
| 1 | Strip model simplification | ✔ Backlogged | DOCUMENTED | None — B-NNN-K stands |
| 2 | Consumption band passive | ⚠ Intentional but incomplete | DOCUMENTED | None — Pattern E + § 14.24 trigger stand |
| 3 | Dispatch error recovery gap | ⚠ Partial fix | **VALID-BUT-BACKLOG** | **NEW B-NNN-R** (caller-side retry helper) |
| 4 | Taper complexity overhead | ⚠ Trade-off | DOCUMENTED | None — § 14.20 complexity intentional |
| 5 | Symmetry weight uncalibrated | ✔ Backlogged | DOCUMENTED | None — B-NNN-Q stands |
| 6 | Taper length heuristic | ✔ Backlogged | DOCUMENTED | None — B-NNN-O stands |

**Web research surface (Rule 7)**: Drawback 3 verified against external API-design literature on retry/recovery contracts:
- AppMaster (API error contract patterns): *"map failures to one of three user actions: fix input, retry, or contact support"* — error contracts give the caller next-step signal; caller decides
- AWS Prescriptive Guidance (circuit breaker pattern): caller-side retry+circuit-breaker is the standard pattern; callee exposes diagnostic metadata, NOT auto-recovery
- Temporal (error handling in distributed systems): retry orchestration is caller's responsibility; "Don't retry client errors — they won't magically fix themselves"

**Net conclusion from web research**: v0.4's § 14.23 design (enriched diagnostics, caller decides) is **architecturally correct per industry consensus**. Reviewer's "not production-smooth" critique is genuine product-quality feedback, but the architectural fix belongs to a caller-side helper (orchestration concern), NOT to C8 itself. B-NNN-R captures this orientation correctly.

**Spec changes**:
- New backlog item B-NNN-R (caller-side retry helper for `CorridorDispatchError`) added in § 12.
- § 9 out-of-scope table: 1 row added.
- § 11 metadata updated for v0.5.
- § 14.23 cross-reference updated to mention B-NNN-R.
- § 16 end-of-spec summary updated.

**No architectural decisions added in v0.5**. No invariant changes. No dataclass changes. **The walk #4 convergence signal is genuine** — v0.5 is the minimal addendum patch, not a substantive iteration.

**Backward-compat from v0.4**: zero behavior change. v0.4 was PROPOSED-not-LOCKED; v0.5 only adds a backlog row + metadata.

---

## § 0.1 — Lineage delta from v0.3 PROPOSED → v0.4 PROPOSED (preserved; historical)
...
class CorridorSegment:
    ...
    config; falls back to `segment_length_m / (2 × max(1, n_tapered_ends))`
    when default exceeds the per-side bound (per Invariant 16 + Invariant 20;
    logged as `"taper_truncated_at_segment_N"` in provenance.rule_trace).
    See § 4.3.1 for the N-aware truncation rule (NEW v0.6).
    ...
    Invariants (asserted at construction):
      ...
      - 2 × taper_zone_m ≤ length_m (Invariant 16; per-side bound, all N)
      - constant_middle_length_m ≥ length_m / 2 (Invariant 20; N-aware
        formula in `constant_middle_length_m` property below)
    ...
    @property
    def n_tapered_ends_actual(self) -> int:                              # NEW v0.6
        """Count of segment ends where width transitions from constant_width_m.

        Returns 0, 1, or 2. Used by `constant_middle_length_m` and the
        § 4.3.1 truncation caller to compute the N-aware bound.
        """
        n = 0
        if abs(self.start_width_m - self.constant_width_m) > EPSILON_M:
            n += 1
        if abs(self.end_width_m - self.constant_width_m) > EPSILON_M:
            n += 1
        return n
    ...
    def has_taper(self) -> bool:
        ...
        return self.n_tapered_ends_actual > 0   # REFINED v0.6 (delegates to N-aware count)

    @property
    def constant_middle_length_m(self) -> float:
        """Length of the constant-width middle region.

        REFINED v0.6: counts only actual tapered ends (where width transitions
        from constant_width_m), not always 2×. For one-end-taper segments
        (dominant case for BRANCH out of single junction) middle is now
        correctly `length - 1 × taper_zone_m`. For two-end-taper segments
        (interior PRIMARY between two width-mismatched junctions) middle is
        `length - 2 × taper_zone_m`. v0.5 always assumed 2×; this caused
        Inv 20 to fail spuriously on the dominant single-junction BRANCH
        case (B-129).
        """
        return max(0.0, self.length_m - self.n_tapered_ends_actual * self.taper_zone_m)
...
#### § 4.3.1 — Taper zone length defaults + truncation fallback (NEW v0.4; REFINED v0.6)
...
def resolve_taper_zone_m(config: CorridorDesignConfig,
                         ...
                         n_tapered_ends: int = 2) -> float:
    """Resolve the taper-zone length for a single segment.

    REFINED v0.6 (B-129 resolution): signature gains `n_tapered_ends`
    parameter ∈ {0, 1, 2} = the count of segment ends where the
    width transitions from `constant_width_m`. Truncation bound is
    now N-aware to keep both Invariant 16 and Invariant 20 satisfiable.
    ...
    Truncation rule (REFINED v0.6):
      max_allowed = segment_length_m / (2.0 × max(1, n_tapered_ends))

      For N=2: max = length / 4 (both tapers consume length / 2 total;
        Inv 20 holds at boundary).
      For N=1: max = length / 2 (one taper consumes length / 2 total;
        Inv 20 holds at boundary; matches v0.5 behaviour).
      For N=0: no taper needed; function returns 0.0 (caller should
        not call this for N=0 segments, but the early-return is a
        defensive safeguard).

    The default `n_tapered_ends = 2` preserves a conservative bound
    for legacy callers that haven't yet been updated to thread the
    actual N through. Such callers will get the tighter bound; this
    is correct-but-restrictive for one-end-taper segments. Code-side
    update to thread N through is tracked as a follow-up patch in § 12.

    Logs "taper_truncated_at_segment_N" in provenance.rule_trace
    when truncation fires.

    Empirical calibration: B-NNN-O (B-123).
    """
    if n_tapered_ends == 0:
        return 0.0  # no taper needed for uniform-width segments
...
- For an N=1 segment shorter than `2 × bay_min`, truncation fires: e.g., a 4m L_SHAPE branch with bay_min = 2.7m → taper truncated to 2.0m per side.
- For an N=2 segment shorter than `4 × bay_min`, truncation fires: e.g., an 8m interior PRIMARY between two width-mismatched junctions with bay_min = 2.7m → taper truncated to 2.0m per side. This is the new bound v0.6 introduces.

**Math justification** (boundary cases, per § 0):

| N | max_allowed | total taper at max | middle at max | Inv 20 (`middle ≥ length/2`) |
|---|---|---|---|---|
| 0 | n/a | 0 | length | ✓ |
| 1 | length / 2 | length / 2 | length / 2 | ✓ (boundary) |
| 2 | length / 4 | length / 2 | length / 2 | ✓ (boundary) |

**Web research grounding (Rule 7, re-confirmed S33)**: NBC 2016 prescribes minimum corridor widths and length-based escalation (`+0.15m per 3m additional length`) but no taper or transition-zone standards for residential corridors. The default `min(bay_x, bay_y)` is a *reasoned default*, not standards-bound. Empirical calibration deferred to **B-123**.

**Constrained-plot override**: if the chosen quantized width pushes corridor outside the buildable envelope (envelope-overflow check at § 4.7 invariant 10), narrow to next-smaller GRID_FRACTION candidate. If even the smallest exceeds `regulatory_min_width_m`-violating, raise `CorridorTooNarrowError(B-109)`.
...
20. **NEW v0.4 — Constant-width-region coverage** (per § 14.20; REFINED v0.6): every segment's constant-width middle region is at least 50% of the segment's length: `constant_middle_length_m ≥ length_m / 2`. The `constant_middle_length_m` formula is N-aware (NEW v0.6): `length_m - n_tapered_ends_actual × taper_zone_m`, where `n_tapered_ends_actual ∈ {0, 1, 2}` counts segment ends where width transitions from `constant_width_m` (per § 3 schema). Ensures taper zones don't dominate the segment. Combined with the N-aware truncation in § 4.3.1, the per-side bound is `taper_zone_m ≤ length_m / (2 × max(1, N))`: `length_m / 2` for N=1 (single-junction segments), `length_m / 4` for N=2 (interior segments between two width-mismatched junctions). Boundary holds for all N.
...
def propagate_junction_widths(segments: list[CorridorSegment],
    ...
    for junction_pt, seg_indices in junction_index.items():
        ...
        #
        # REFINED v0.6: thread the actual `n_tapered_ends` count (post-this-
        # update) into resolve_taper_zone_m so the truncation rule sees the
        # correct N. This is computed by counting how many ends of the segment
        # will have a width transition AFTER this propagation pass:
        # - if the other end of the segment is also a junction with a
        #   different inherited width than constant_width_m, N=2;
        # - otherwise, N=1 for this single-junction-touching pass.
        for i in seg_indices:
            seg = segments[i]
            n_tapered = _count_actual_tapered_ends_after_pass(  # NEW v0.6 helper
                seg, junction_index, segments,
            )
            taper_m = resolve_taper_zone_m(
...
**Worked example A** (L_SHAPE, PRIMARY=1.65m, BRANCH=1.20m, bay_min=3.3m, BRANCH length=5m, single junction at start of BRANCH; N=1):
...
| BRANCH taper out of junction | (no taper; uniform 1.65m) | tapers from **1.65m** → 1.20m over `taper_zone_m = 2.5m` (truncated from proposed 3.3m by § 4.3.1; N=1 → max=length/2=2.5m) |
| BRANCH constant middle | 1.65m | **1.20m** (back to requested width) |
| BRANCH far-end taper | (none) | (none; far end is not a junction; N for BRANCH = 1) |

**Inv 20 check (BRANCH, N=1)**: middle = 5 − 1×2.5 = 2.5m ≥ 5/2 = 2.5m ✓ (boundary).

(Note: v0.5 spec text reported `taper_zone_m = 3.3m` here without applying the v0.5 truncation; v0.6 corrects this — proposed 3.3m exceeds N=1 max=2.5m, so truncation fires. v0.5 production code already truncated correctly; only the spec-table label was stale.)

**Net effect (v0.6 unchanged from v0.4 intent)**: BRANCH constant middle is 1.20m as requested (not v0.3's inflated 1.65m). Wide-PRIMARY corridor area inflation is contained to the taper zone adjacent to the junction.

**Worked example B (NEW v0.6)** — interior PRIMARY between two width-mismatched junctions; N=2:

Configuration: T-shape with PRIMARY-A=1.65m running E-W joining at junction J1 (with PRIMARY-B=1.20m branching south) and at junction J2 (with PRIMARY-C=1.50m branching north). PRIMARY-A length = 7.0m, bay_min = 3.3m. Both ends of PRIMARY-A are junctions where the other adjoining segment has a different `constant_width_m` from PRIMARY-A's 1.65m, so PRIMARY-A's N=2 (well, junction-max width applies; the relevant test is whether width transitions at each end). Specifically: junction J1 inherits width 1.65m (= max(1.65, 1.20)) which matches PRIMARY-A's constant; junction J2 inherits 1.65m (= max(1.65, 1.50)) which also matches. So actually for PRIMARY-A's two ends, both end widths equal constant_width_m (no taper). N for PRIMARY-A = 0.

The N=2 case arises for the *junior* segment in such a junction: e.g., PRIMARY-B (1.20m) at J1 inherits 1.65m at start; if PRIMARY-B's other end is also a junction where it inherits, say, 1.50m, then PRIMARY-B has start_width=1.65m, constant=1.20m, end_width=1.50m — N=2.

Configuration revised: take a junior interior segment SEG with constant=1.20m, start_width=1.65m (from J1), end_width=1.50m (from J2), length=7.0m, bay_min=3.3m.

| Quantity | Value (v0.6) |
|---|---|
| n_tapered_ends_actual | 2 (both ends transition from constant 1.20m) |
| Proposed taper_zone_m | min(bay_x, bay_y) = 3.3m |
| § 4.3.1 max_allowed (N=2) | 7.0 / 4 = 1.75m |
| Truncation fires? | Yes (3.3 > 1.75) → taper_zone_m = 1.75m |
| Total taper consumed | 2 × 1.75 = 3.5m |
| constant_middle_length_m | 7.0 − 2 × 1.75 = 3.5m |
| Inv 20 check | 3.5 ≥ 7.0/2 = 3.5 ✓ (boundary) |

**Where v0.5 would have failed**: v0.5 § 4.3.1 max_allowed = 7.0 / 2 = 3.5m. With proposed 3.3m < 3.5m, no truncation. Total taper = 2 × 3.3 = 6.6m. Middle = 7.0 − 6.6 = 0.4m. Inv 20: 0.4 ≥ 3.5? **NO** — violation by 3.1m. v0.5 build would have raised an Inv 20 assertion error here. v0.6's N-aware truncation catches it.
...
| **NEW v0.5: Caller-side retry helper for `CorridorDispatchError`** (orchestration layer, not C8) | **B-NNN-R** |
...
- **Version**: v0.5 LOCKED
- **Status**: LOCKED by Ramalingam at S31 close. Build-ready.
- **Lineage**: v0.1 DRAFT (S31) → v0.2 PROPOSED (S31; walk #1 + V-A) → v0.3 PROPOSED (S31; walk #2 + 9 ADs) → v0.4 PROPOSED (S31; walk #3 + 6 ADs § 14.19–§ 14.24 + 3 new B-NNNs) → v0.5 PROPOSED (S31; walk #4 addendum + 1 new B-NNN-R) → **v0.5 LOCKED (S31 close; reviewer-confirmation walk #5; B-NNN concrete-number assignments)**
- **LOCK session**: S31
- **Companion artifacts**: C5 SPEC v0.9 LOCKED; C6 SPEC v0.7 LOCKED; C7 `Grid` schema (verified S31)
- **Web research surface (Rule 7)**: NBC + TNCDBR + Neufert + industry-gridline-practice (v0.2). v0.3 walk: column-line = structural-line consensus (§ 14.13). v0.4 walk: residential corridor over- and under-design BOTH have real costs (§ 14.19). **v0.5 walk: caller-side retry / circuit-breaker is industry-consensus pattern (AWS Prescriptive Guidance, AppMaster API error contracts, Temporal distributed-systems error handling). Verifies § 14.23 design correctness; B-NNN-R captures orchestration-layer concern correctly.**
- **Empirical verification at S31** (Rule 7 mandate): no new empirical work in v0.5 (walk #4 had no code-claims requiring grep). v0.4 verifications still hold (scored width selection, trapezoid sweep-line union on 6 cases, decomposition arithmetic).

---

## § 12 — Backlog enumeration (REFRESHED v0.6)

Total: **19 v0.5-era items + 3 v0.6-era items (B-129, B-130, B-131) = 22 items.** All v0.5-era B-NNNs received concrete assignments at v0.5 LOCK (B-108 through B-126). v0.6 references the new entries and notes B-129's in-spec resolution.

### v0.6 entries (NEW)

#### B-129 — § 4.3.1 truncation rule vs Inv 20 inconsistency — **RESOLVED IN-SPEC v0.6**

**Origin**: S32 C8 build. § 4.3.1 (`max_allowed = length / 2`) and Inv 20 (`middle ≥ length / 2`) were mutually inconsistent on segments with two tapered ends; the v0.5 schema's `constant_middle_length_m = length - 2 × taper_zone_m` always assumed both ends taper, undercounting middle for the dominant single-junction BRANCH case.

**Resolution path actually taken in v0.6**:
- (a) Schema's `constant_middle_length_m` is N-aware (counts actual tapered ends via `n_tapered_ends_actual` property; see § 3).
- (b) § 4.3.1 truncation is N-aware (`max_allowed = length / (2 × max(1, N))`); see § 4.3.1.
- (c) § 4.10 worked example A (N=1) re-verified; new worked example B (N=2) added.
- (d) Inv 20 text refined to drop the "(assuming both ends taper)" parenthetical and reference the N-aware formula.

**Note on B-129's original sketch**: the resolution path drafted in the v9-bundle backlog suggested `length / (2 + 2 × N)`. That formula was incorrect (would change the v0.5 § 4.10 outcome and unnecessarily restrict N=1 segments). v0.6 uses `length / (2N)` which is the necessary and sufficient condition derived from Inv 20.

**Status**: RESOLVED-IN-SPEC v0.6 (PROPOSED). Backlog entry will close at v0.6 LOCK.

#### B-130 — Replace 1cm rasterization in `polygon_union_area_m2` with spec § 4.8 sweep-line

**Origin**: S32 C8 build, `area_accounting.py`. v1 implementation uses 1cm grid rasterization vs the spec § 4.8 prescribed sweep-line algorithm. Correctness verified against all 3 spec verification cases; rasterization error ≤ 0.01 m² per segment (well within tolerance) and N is small in current pipeline.

**Status**: BACKLOG (architecture/performance; not blocking; correctness preserved).

**Description**: Re-implement using the sweep-line algorithm specified in § 4.8. Maintain backward-compatible public signature. Add benchmarks comparing v1 rasterization vs sweep-line against synthetic large-N corridor sets to characterize the regime where sweep-line is justified.

**Trigger**: Future C8 spec walk prioritizes spec-fidelity over implementation simplicity, OR downstream component generates corridor topologies with large N where rasterization cost becomes meaningful, OR a performance benchmark surfaces > 100ms latency in `polygon_union_area_m2()`.

**S33-scope verdict**: OUT — filed at S33 open per Ramalingam directive; correctness preserved; defer until trigger.

**Effort**: M (~1 session for sweep-line + benchmarks + correctness re-verification).

#### B-131 — Code-side patch: thread `n_tapered_ends` through `resolve_taper_zone_m()` callers

**Origin**: v0.6 § 4.3.1 amendment. The v0.5 production code's callers of `resolve_taper_zone_m()` (in `width_selection.py` and `junction_propagation.py`) pass only the v0.5 single-arg signature. Under v0.6, `resolve_taper_zone_m()` accepts `n_tapered_ends` with default = 2 (conservative); legacy callers still pass no value, so they get the conservative bound (correct-but-restrictive for N=1 segments). Tighten by threading the actual N through.

**Status**: BACKLOG (code-side follow-up to v0.6 spec amendment).

**Description**: After v0.6 LOCK, update `width_selection.py` and `junction_propagation.py` to compute `n_tapered_ends` per segment and pass it. The computation is non-trivial because at the time `resolve_taper_zone_m()` is called for a junction-end, the *other* end's status (junction or free) determines whether N=1 or N=2 for this segment globally. Two-pass algorithm: first identify all segments whose constants will deviate from inherited junction widths; then resolve N per segment; then call resolver. Tests: (a) regress § 4.10 worked example A (N=1) with explicit n_tapered_ends=1 produces taper=2.5m; (b) new test reproduces § 4.10 worked example B (N=2) producing taper=1.75m.

**Trigger**: Concurrent with v0.6 LOCK + code patch round (D-066 step 8).

**S33-scope verdict**: IN-SCOPE-AT-LOCK — filing now to schedule alongside the LOCK adjudication; not in v0.6 PROPOSED itself.

**Effort**: S (~1-2 hours for the threading + test additions).

---

### v0.5-era entries (carried forward; B-NNNs concrete per v0.5 LOCK)

Total: **19 items**. All OUT-of-scope this build.
...
### B-NNN-R — Caller-side retry helper for `CorridorDispatchError` — NEW v0.5

**Origin**: C8 v0.5 critique walk #4 Drawback 3.
**Status**: BACKLOG (downstream-component-orchestration concern; NOT a C8 spec defect).
**Description**: When a higher-level pipeline orchestrator receives `CorridorDispatchError` from C8, it currently must hand-roll the retry logic — try the next `oriented_candidate`, surface the error to the user, abort the pipeline. v0.4's § 14.23 enriched-diagnostic contract makes this *possible* via the metadata fields (`candidate_index`, `topology_kind`, `failure_phase`, `suggested_alternative_topologies`) but doesn't make it *easy*. A `retry_with_next_candidate()` helper utility would standardize the retry contract across pipeline consumers.

**Why this is NOT a C8-internal concern** (per industry consensus on retry/circuit-breaker patterns, verified S31 web research): recovery orchestration belongs to the *caller*, not the *callee*. AWS Prescriptive Guidance (circuit breaker pattern), AppMaster (API error contracts), and Temporal (distributed-system error handling) all converge: the callee exposes diagnostic metadata; the caller decides retry policy. v0.4's § 14.23 contract follows this pattern correctly. Adding the helper to C8 would couple C8 to caller-side retry policy (Pattern E risk — and the Walk #2 Drawback 8 / § 14.17 pushback against soft fallback applies here too).

**Where the helper belongs**: in a higher-level pipeline-orchestration component (likely C14 or a new pipeline orchestrator), NOT in C8. The helper consumes C8's enriched error contract; it doesn't change it.

**Trigger**: First higher-level pipeline component that consumes C8 output and needs to handle dispatch failures gracefully (likely C14 or pipeline orchestrator at integration time).
**S31-scope verdict**: OUT — pipeline-orchestration concern, not C8-build-time. Filed as a C8 backlog item (per Ramalingam direction at walk #4 close, option B) for full audit-trail visibility, even though the eventual implementation will be in a different component.
**Effort**: S (the helper itself is a small wrapper; the orchestrator that hosts it is separate work).

---

**Total v0.5 backlog**: **19 items** (v0.1: 7; v0.2: +4 = 11; v0.3: +4 = 15; v0.4: +3 = 18; v0.5: +1 = 19). All OUT-of-scope this build.

**Total v0.6 additions**: **3 items** (B-129 RESOLVED-IN-SPEC; B-130 NEW BACKLOG; B-131 NEW SCHEDULED-AT-LOCK). v0.6 grand total = 22.

### v0.6 backlog summary table

| ID | Title | Origin | Status | Trigger | S33-scope | Effort |
|---|---|---|---|---|---|---|
| B-129 | § 4.3.1 vs Inv 20 inconsistency | S32 build | RESOLVED-IN-SPEC v0.6 | Closed at v0.6 LOCK | n/a | n/a (resolved in spec) |
| B-130 | Sweep-line union vs 1cm rasterization | S32 build | BACKLOG | Future spec walk OR perf surface | OUT | M |
| B-131 | Thread n_tapered_ends through resolve_taper_zone_m callers | v0.6 amendment | BACKLOG (code-side) | At v0.6 LOCK code-patch round | IN-AT-LOCK | S |
...
**Caller-side retry-helper utility (v0.5 update)**: walk #4 reviewer correctly noted that v0.4's contract makes retry *possible* but not *easy* — every caller hand-rolls the retry logic. The architectural fix is a `retry_with_next_candidate()` helper utility in a higher-level orchestration component (NOT in C8). Per industry retry-pattern consensus (AWS, AppMaster, Temporal — verified S31 web research), recovery orchestration belongs to the caller. C8's contract is correct as-is; the helper is a downstream concern. Filed as **B-NNN-R**.
...
| **Walk #4 Drawback 1: Strip model simplification** | v0.5 critique walk | DOCUMENTED — reviewer confirmed B-NNN-K handling correct; no v0.5 action |
| **Walk #4 Drawback 2: Consumption band passive** | v0.5 critique walk | DOCUMENTED — reviewer confirmed Pattern E + § 14.24 handling; no v0.5 action |
| **Walk #4 Drawback 3: DispatchError no recovery (still)** | v0.5 critique walk | VALID-BUT-BACKLOG — § 14.23 contract correct per industry consensus (AWS, AppMaster, Temporal); helper belongs to orchestration layer → **B-NNN-R** |
| **Walk #4 Drawback 4: Taper complexity overhead** | v0.5 critique walk | DOCUMENTED — reviewer confirmed § 14.20 trade-off is intentional; no v0.5 action |
| **Walk #4 Drawback 5: Symmetry weight uncalibrated** | v0.5 critique walk | DOCUMENTED — already B-NNN-Q; no v0.5 action |
| **Walk #4 Drawback 6: Taper length heuristic** | v0.5 critique walk | DOCUMENTED — already B-NNN-O; no v0.5 action |
...
## § 16 — End of v0.5 LOCKED

**LOCK declaration**:

```
Type           : v0.5 LOCKED (Ramalingam adjudication at S31 close)
Predecessor    : v0.5 PROPOSED → walk #5 reviewer confirmation → LOCK
Authority      : Ramalingam (Rule 8 — LOCK is Ramalingam's call alone)
LOCK session   : S31
Build-ready    : YES
Code           : NOT YET STARTED — first build session pending
B-NNN status   : All placeholders A-R assigned concrete numbers (B-108..B-126)
                 B-110 already concrete from v0.2.
                 Pre-LOCK backlog max: B-107.
                 Post-LOCK backlog max: B-126.
Backlog count  : 19 items, all OUT-of-scope-this-build, all with documented
                 trigger conditions. Mirror in 04_backlog/v0_2_backlog.md.
ADs total      : 24 (5 v0.1 preserved + 9 v0.2 + 9 v0.3 + 6 v0.4 + 0 new v0.5)
Invariants     : 20 total
Web research   : Verified per Rule 7 across all 5 walks
Empirical work : Verified per Rule 7 — bay-size bias, FP accumulation,
                 trapezoid union, decomposition arithmetic
Walks          : 5 total (#1-#3 substantive; #4 minor addendum;
                 #5 confirmation)
Convergence    : Achieved at walk #4 — walk #5 produced acknowledgments
```

**Convergence trajectory** (full history through LOCK):

```
                       reviewer items    new B-NNNs    outcomes
v0.1 → v0.2                7 critiques       4         6 RESOLVED, 1 PARTIAL, 1 V-A self-found
v0.2 → v0.3                9 critiques       4         4 RESOLVED, 5 PARTIAL
v0.3 → v0.4                7 critiques       3         4 RESOLVED, 2 PARTIAL, 1 AFFIRMED
v0.4 → v0.5                6 critiques       1         5 DOCUMENTED, 1 VALID-BUT-BACKLOG
v0.5 → LOCK                6 acknowledgments 0         all reviewer-confirmed-handled
```

**LOCK is final per Rule 8.** Adjudication-window critique no longer applies; further refinement happens via:
- B-108..B-126 backlog items (pre-build calibration / post-build empirical refinement)
- C8 build-session work (test infrastructure, code, no spec changes unless a build-time discovery breaks an invariant)
- Future spec versions (v1.0+) only after C8 SHIPs and field signal justifies revision

**Build phase begins next session.** First build steps per D-066 cycle:
1. Skeleton: dataclasses + module structure mirroring C7's component layout
2. Failure modes first (TDD shape from § 6)
3. Spatial model (§ 4.0) → topology dispatch (§ 4.1) → grid lines (§ 4.2)
4. Width selection (§ 4.3) → endpoint construction (§ 4.4) → spatial feasibility (§ 4.7)
5. Junction propagation (§ 4.10) — the complex piece per Ramalingam's walk #3 directive
6. Area accounting (§ 4.8) — trapezoid sweep-line, decomposition verified
7. Validator (§ 4.6 — 20 invariants tiered ALL-PATHS / HAS-CORRIDOR-ONLY)
8. Provenance + public API (§ 5)
9. Test corpus (~155 tests across 9 files per § 7)

**End of C8 v0.5 LOCKED.**
