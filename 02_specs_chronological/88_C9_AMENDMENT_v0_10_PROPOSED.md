# C9 SPEC AMENDMENT v0.10 PROPOSED — `has_master_bedroom` flag on `FloorRoomBrief` (B-NEW-T3 enabler #2 of 4)

**Component**: 9 (Room Sizing — SHIPPED at v0.7 LOCKED).
**Spec status**: **v0.10 PROPOSED. PENDING Ramalingam LOCK adjudication.**
**Authority**: Ramalingam directive at S40-continuation: spec-first, four-spec sequence for B-NEW-T3.
**Authored**: S40-continuation, post-v0.9 critique walk.
**Driver**: Spec #2 of 4 — directly required by Spec #1 (`MultiFloorDwellingBrief`) Inv MFDB-6 coupling contract.

**Note on v0.10 vs v0.9**: second documentation-polish round. **No schema or runtime changes.** v0.10 patches are wording adjustments + 1 new backlog filing applied during the v0.9 critique walk. Functionally and structurally identical to v0.9.
...
NEVER self-declares LOCK. This file is `v0.10 PROPOSED`. After Ramalingam
adjudication, the next file in `02_specs_chronological/` will be either:
- `..._v0_10_LOCKED.md` (if approved as-is),
- `..._v0_11_PROPOSED.md` (if patches surfaced — Rule 8 patch-eligible
  until LOCK).
...
## § 0.1 — Delta from v0.9 → v0.10 (critique walk patches)

v0.9 PROPOSED received an external critique with 12 items. The reviewer noted
"this is genuinely approaching LOCK quality" and "LOCK READINESS: very high.
Remaining critique surface is now mostly philosophical architecture
direction... runtime design itself appears stable." Verdicts:

| Item | Verdict | Patch site |
|---|---|---|
| CRITICAL-1 (precedent for embedding orchestration metadata into domain) | ACCEPT | § 3.9 — precedent-narrowing sentence |
| MAJOR-1 (trust-boundary observability timing undefined) | ACCEPT | § 3.10 + § 6 — assertion lifecycle clarifier |
| MAJOR-2 (prose authority outpaces runtime enforcement) | BACKLOG ONLY | New B-C9-E |
| MAJOR-3 (two overlapping canonicality systems) | REJECT | None — already covered by B-MFDB-O + B-C9-D + B-MFDB-I collectively |
| MAJOR-4 (default-True asymmetry) | REJECT | None — already documented in § 3.1 v0.9 |
| MODERATE-1 (cache rationale outpaces cache architecture) | REJECT | None — non-actionable maturity signal |
| MODERATE-2 ("WILL own" creates inter-spec coupling pressure) | ACCEPT | § 6 — softener "is expected to own" |
| MODERATE-3 (B-C9-D more strategically important than label suggests) | ACCEPT | B-C9-D trigger re-tag (drop "post-v1" framing, name action triggers) |
| MODERATE-4 (semantic density vs implementation size) | REJECT | None — non-actionable maturity signal |
| MINOR-1 ("future architecture may..." prose density) | REJECT | None — forward-compat warnings are intentional value |
| MINOR-2 ("orchestration-contextual" framing may discourage reuse) | ACCEPT | § 3.9 — deterministic-and-structurally-valid clarifier |
| MINOR-3 (convergence note slightly optimistic) | REJECT | None — note is accurate for what it claims |

**Net schema delta from v0.9**: NONE. v0.10 adds zero new fields, runtime logic, validations, or tests. Five wording adjustments + 1 new backlog filing.

Six rejects with reasoning preserved (MAJOR-3, MAJOR-4, MODERATE-1, MODERATE-4, MINOR-1, MINOR-3) + 1 backlog-only downgrade (MAJOR-2).

**Convergence signal**: v0.8 → v0.9 had no schema changes. v0.9 → v0.10 also has no schema changes. Spec #2 has structurally converged. Five of six accepted patches are single-sentence wording adjustments.
...
- **v0.10 PROPOSED. PENDING Ramalingam LOCK adjudication.**
- Authority: Rule 8 — LOCK authority belongs to Ramalingam alone.
- Patch-eligibility: critique surfaced before LOCK produces v0.11 PROPOSED.
...
- **Convergence note**: v0.8 → v0.9 was the first round with NO schema or runtime changes. v0.9 → v0.10 is the second consecutive round with NO schema or runtime changes. Spec #2 has structurally converged. Five of six accepted v0.10 patches are single-sentence wording adjustments; the sixth (B-C9-E filing) is a backlog item, not a spec change.
...
**Summary**: smallest of the four B-NEW-T3 specs. One field, two line-edits, one constant bump. Five C9-amendment-specific backlog items (B-C9-A through E). Pre-existing C9 backlog unaffected.

---

**End of C9 Amendment v0.10 PROPOSED.**
