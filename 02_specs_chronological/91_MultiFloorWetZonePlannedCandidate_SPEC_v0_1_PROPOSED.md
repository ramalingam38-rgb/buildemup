# MULTI-FLOOR WET-ZONE PLANNED CANDIDATE SPEC v0.1 PROPOSED — output-side multi-floor wrapper (B-NEW-T3 enabler #3 of 4)
...
**Authored**: S40-continuation, post-Spec-#2 LOCK.
**Driver**: Spec #3 of 4. Output-side counterpart to Spec #1 (`MultiFloorDwellingBrief`). Defines what a multi-floor C10/C11a candidate looks like as a single domain object, so Spec #4 (C11a v1.1) can wire multi-floor candidates through the pipeline.
...
class MultiFloorWetZonePlannedCandidate:
    ...
    def _derive_floor_labels(self) -> tuple[str, ...]:
        ...
        Path: WZPC.room_sized_candidate.provenance.floor_room_brief.floor_label

        Note: the brief is already-normalized (Spec #1 § 3.1
        nested-rewrite ensured per-floor briefs were normalized at
        MultiFloorDwellingBrief construction time, and those briefs
        flowed through C9/C10 unchanged). So derived labels are
        canonical without further normalization.
        """
        labels = []
        for i, f in enumerate(self.floors):
            try:
                brief = f.room_sized_candidate.provenance.floor_room_brief
                labels.append(brief.floor_label)
            except AttributeError as e:
                raise ValueError(
                    f"MultiFloorWetZonePlannedCandidate.floors[{i}]: cannot "
                    f"derive floor_label from ancestry chain "
                    f"(room_sized_candidate.provenance.floor_room_brief.floor_label). "
...
- Patch-eligibility: critique surfaced before LOCK produces v0.2 PROPOSED.
...
**End of MultiFloorWetZonePlannedCandidate Spec v0.1 PROPOSED.**
