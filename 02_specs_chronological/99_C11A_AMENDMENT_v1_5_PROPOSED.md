# C11A SPEC AMENDMENT v1.5 PROPOSED — multi-floor pipeline rework (B-NEW-T3 enabler #4 of 4)
...
**Spec status**: **v1.5 PROPOSED. PENDING Ramalingam LOCK adjudication.**
**Authority**: Ramalingam directive at S40-continuation: spec-first, four-spec sequence for B-NEW-T3.
**Authored**: S40-continuation, post-v1.4 critique walk.
**Driver**: Spec #4 of 4, the largest. Completes the B-NEW-T3 spec sequence.

**v1.5 vs v1.4**: first non-behavioral-change round since v1.1. **Two pre-LOCK contract/wording patches** (split `affected_floor_set` API to disambiguate `direct_floor_label` vs `new_master_floor_label`; replace overstated "uniform under truncation" fairness wording with mathematically-precise "bounded imbalance ≤ 1, starvation-free"). Plus four documentation polish patches (family-ID vs wrapper semantic-divergence callout, cyclic coverage off-by-one fix, drift taxonomy root-cause-precision caveat, new § 3.4.2 generation contract). Zero new backlog filings; the v1.4 critique walk's 13 backlog-applicable items were all already filed.
...
NEVER self-declares LOCK. This file is `v1.5 PROPOSED`. After Ramalingam
adjudication, the next file in `02_specs_chronological/` will be either:
- `..._v1_5_LOCKED.md` (if approved as-is),
- `..._v1_6_PROPOSED.md` (if patches surfaced — Rule 8 patch-eligible
  until LOCK).
...
- **v1.5 PROPOSED. PENDING Ramalingam LOCK adjudication.**
- Authority: Rule 8 — LOCK authority belongs to Ramalingam alone.
- Patch-eligibility: critique surfaced before LOCK produces v1.6 PROPOSED.
- After LOCK: **B-NEW-T3 build can begin** (all four specs LOCKED).
- **Convergence note**: v1.5 is the first non-behavioral-change round since v1.1, addressing the v1.4 reviewer's two explicitly-named pre-LOCK items (`affected_floor_set` semantic overload + overstated "uniform" fairness wording). Reviewer's overall v1.4 verdict ("Convergence assessment: VERY HIGH. Most likely trajectory: v1.5 PROPOSED, then LOCK.") signals this is plausibly the final pre-LOCK round. The remaining v1.4 critique items were either backlog-applicable (all already filed in previous rounds; v1.5 adds zero new backlog items) or documentation polish (items 4, 5, 6, 13 — all landed). Three-consecutive-behavioral-change run (v1.1 → v1.2 → v1.3 → v1.4) is complete; v1.5 is the doc-polish round before LOCK that mirrors how Specs #1, #2, #3 each reached LOCK quality.
...
**Summary**: largest of the four B-NEW-T3 specs. Threads multi-floor through every C11a pipeline stage. **15 total backlog items** (B-C11A-1 through 15) across four critique-walk rounds; v1.5 adds zero new items (all v1.4 critique walk's backlog-applicable items were already filed in previous rounds). v1.5 addresses the two pre-LOCK items reviewer named in the v1.4 critique walk: `affected_floor_set` API split (item 1) and fairness wording precision (item 2). After LOCK, the B-NEW-T3 build session begins — the largest implementation work item in S41+.

---

**End of C11a Amendment v1.5 PROPOSED.**
