# BuildemUp — C8 Corridor Designer — SPEC v0.5 PROPOSED

**Status**: PROPOSED. PENDING critique walks → LOCK adjudication. Critique-walk-#4 addendum patch over v0.4 PROPOSED. Walk #4 produced 6 reviewer items: 5 reviewer-confirmed-already-handled (acknowledgments of v0.4's handling); 1 genuinely actionable (Drawback 3 → B-NNN-R, caller-side retry helper). Net spec change is small (one new backlog item + supporting metadata updates) — convergence signal achieved.
...
**Authoring session**: S31 (post C6 SHIP, post v0.4 critique walk #4).
**Predecessor**: v0.4 PROPOSED (S31; critique walk #3).
...
- **Status**: PROPOSED. PENDING Ramalingam LOCK adjudication; not yet build-ready. Walk #4 addendum patch over v0.4 — minimal scope.
- **Lineage**: v0.1 DRAFT (S31) → v0.2 PROPOSED (S31; walk #1 + V-A) → v0.3 PROPOSED (S31; walk #2 + 9 ADs) → v0.4 PROPOSED (S31; walk #3 + 6 ADs § 14.19–§ 14.24 + 3 new B-NNNs) → **v0.5 PROPOSED (S31; walk #4 addendum + 1 new B-NNN-R)**
...
```
Type           : v0.5 PROPOSED (critique walk #4 addendum patch over v0.4)
Driver         : 6-item reviewer critique walk #4 + Ramalingam adjudication
                 (option B: file B-NNN-R as proper C8 backlog item before
                 LOCK rather than as post-LOCK project-backlog item)
Authoring      : S31
Web research   : Walk #4 Drawback 3 verified against industry retry/recovery
                 contract literature (AWS, AppMaster, Temporal); confirms
                 v0.4's § 14.23 design as architecturally correct
Code-grep      : not applicable (walk #4 had no code-claims requiring grep)
Reviewer       : 5 DOCUMENTED-no-action (1, 2, 4, 5, 6)
verdicts         1 VALID-BUT-BACKLOG (3 → B-NNN-R)
                 0 RESOLVED-by-spec-change (none needed)
                 0 PARTIAL+PUSHBACK (architecturally clean)
                 0 misframed
Empirical fix  : none — v0.5 is metadata-only
New ADs        : 0
v0.4 ADs       : 6 preserved (§ 14.19 through § 14.24); § 14.23
                 cross-references B-NNN-R for caller-side helper
v0.3 ADs       : 9 preserved (§ 14.10 through § 14.18)
v0.2 ADs       : 9 preserved (§ 14.1 through § 14.9)
v0.1 ADs       : 5 preserved (§ 14.1.1–14.1.5), 3 superseded
New B-NNNs     : 1 (B-NNN-R)
Total backlog  : 19 items (was 18 in v0.4)
New invariants : 0 (still 20 total)
Behavior change: ZERO. v0.5 is the minimal addendum — backlog row +
                 supporting metadata updates. v0.4 was PROPOSED-not-LOCKED;
                 no backward-compat issues.
Status         : PROPOSED. PENDING Ramalingam LOCK adjudication.
```

**Convergence trajectory** (full history):
...
v0.4 → v0.5                6 critiques       1              0
                           5 DOCUMENTED-no-action          
                           1 VALID-BUT-BACKLOG               
```

**Convergence reading**: walk #4 produced 5 acknowledgments + 1 actionable backlog entry, no new architectural decisions, no new invariants, no schema changes. The walk #4 reviewer explicitly concluded: *"v0.4 is very close to LOCK quality... no architectural blockers remain."* v0.5's spec deltas are minimal and the trajectory has flattened. **v0.5 is LOCK-ready.**

Per Rule 8: **PROPOSED. PENDING Ramalingam LOCK adjudication.** Adjudication-window critique on v0.5 stays patch-eligible (would produce v0.6 PROPOSED) until you say "lock it."

**Awaiting**: LOCK or one final confirmation walk.
