# BuildemUp — C6 Orientation Priority — SPEC v0.7 PROPOSED

**Status**: PROPOSED. PENDING Ramalingam LOCK adjudication. Mid-session critique-walk patch over v0.6 LOCKED — applies one consolidation patch (new § 14.21) per S31 D-066 Step 7 reviewer feedback. Zero behavior change.
...
**Lineage delta from v0.6 LOCKED → v0.7 PROPOSED** (one discrete change, mid-session critique-walk consolidation patch; D-066 Step 7):

1. **Critique-walk-1 — Architectural-invariant consolidation** (new § 14.21). The S31 D-066 Step 7 reviewer flagged that the cardinal-only external contract (B-107) and the internal 8-direction Vastu dependency form an architectural asymmetry that, while documented in 35+ spec/code locations, is nowhere consolidated as a single first-class invariant. Verdict: PARTIALLY VALID — the asymmetry is real and the consolidation is worthwhile; pushback on the framing ("silent mismatch" overstates a 35-location distribution) and on the proposed "OCTILE" terminology (not project vocabulary). v0.7 adds a single consolidated § 14.21 block + matching code-level cross-references in `schema.py:CARDINAL_FACINGS`, `select.py:_validate_session`, and `vastu_kb.py` module docstring. **Zero behavior change.** No new B-NNN. No new tests required.

**Lineage delta from v0.5 LOCKED → v0.6** (three discrete changes; all from S31 pre-code Q&A):
...
- **Status**: PROPOSED. PENDING Ramalingam LOCK adjudication. Mid-session critique-walk patch (D-066 Step 7).
- **Lineage**: v0.1 DRAFT → v0.2 PROPOSED (walk #1) → v0.3 PROPOSED (walk #2) → v0.4 PROPOSED (walk #3) → v0.5 PROPOSED (walk #4) → v0.5 LOCKED (Ramalingam, S30 close) → v0.6 PROPOSED (S31 pre-code patch round) → v0.6 LOCKED (Ramalingam, S31) → **v0.7 PROPOSED (S31 D-066 Step 7 critique-walk consolidation patch)**
...
### From v0.7 (S31 D-066 Step 7 critique-walk consolidation patch — NEW)

#### § 14.21 — Architectural invariant: internal 8-direction Vastu vs external cardinal contract

**Stated as a first-class invariant** (consolidating what v0.6 distributed across 35+ locations):

> **C6 v1 operates on two distinct direction models, intentionally asymmetric:**
> - **External contract** (4-direction, cardinal): C6 accepts only `plot.facing ∈ {NORTH, EAST, SOUTH, WEST}`; rejects intercardinal at the input boundary with `NotImplementedError(B-107)`. C6's *output* (`direction_priorities`, `refined_zone_bands`) is also restricted to those four cardinals (validator invariants 1 + 9).
> - **Internal Vastu computation** (8-direction): the PARTIAL-tier Vastu KB is keyed by all 8 `PlotOrientation` members (cardinal + intercardinal). The 4-direction cardinal scores consumed by the optimizer are derived via weighted-average aggregation over each cardinal + its two intercardinal neighbours (§ 4.1.4). Without intercardinal data, the cardinal scores would lose roughly half their fidelity (e.g., NORTH would not benefit from the high NE/POOJA Ishaan signal).

**Why both layers exist together (not a defect, not an oversight)**:

1. Vastu's signature placements (Ishaan/NE for pooja, Agni/SE for kitchen, etc.) live at intercardinal positions in the canonical Vastu Purusha Mandala. Collapsing the internal table to 4-direction would erase those signals — a real product harm.
2. Sun and wind scoring tables, by contrast, can reasonably stay 4-direction in v1 because their physical gradients are smoother (sun is a continuous function of azimuth; the cardinal samples are representative). They would need extension when B-107 lands but are not currently bottlenecked by it.
3. The optimizer (`enumerate_permutations`, `prune_by_entry_on_road`, scoring, tie-break, hysteresis) operates entirely in the 4-direction cardinal space — extending it to 8-direction is the substantive design dialogue B-107 covers.

**Enforcement points** (where the cardinal-only external contract is asserted):

| Location | What it asserts |
|---|---|
| `select.py:_validate_session` | Rejects intercardinal `plot.facing` at input boundary (B-107 message) |
| `schema.py:CARDINAL_FACINGS` | The named frozenset used throughout; single source of truth for "which directions are cardinal" |
| `OrientationPriority.__post_init__` invariant 1 | `direction_priorities` keys must be exactly `CARDINAL_FACINGS` |
| `OrientationPriority.__post_init__` invariant 9 | `refined_zone_bands` values must all be in `CARDINAL_FACINGS` |

**Internal-8-direction touchpoints** (where intercardinal data is still consumed):

| Location | What uses it |
|---|---|
| `vastu_kb.py:VASTU_TABLE_PARTIAL` | 8-keyed mapping; all 8 PlotOrientation members present |
| `vastu_kb.py:vastu_score_4dir` | Reads cardinal + 2 intercardinal entries; weighted-avg aggregation |
| `vastu_kb.py:adjacent_intercardinals` | Returns the (CCW, CW) intercardinal neighbours of a cardinal |

**When B-107 lands** (intercardinal external support):

- The "External contract" line above becomes 8-direction.
- The two enforcement points (validator invariants 1 + 9, the `_validate_session` check) get loosened.
- The "Internal-8-direction touchpoints" stay roughly as they are (the data was always there).
- The substantive new work is on the *external scoring side* (sun + wind 8-row tables) and the *optimizer keyspace* (Hamming over 8 directions, permutation cap re-validation against the new 4^8 / 8! ceilings) — both already enumerated in B-107's effort estimate.

**Why this consolidation was not in v0.6**: the v0.6 patch round was driven by a pre-code Q&A that surfaced three concrete defects (Q1-A enum drop, Q1-B intercardinal handling, Q2 CIRCULATION type-fix). The architectural-invariant consolidation is a layer of meta-documentation that became visible only when reviewers read the whole v0.6 build end-to-end. Adding it post-LOCK as v0.7 is the proportionate fix. Zero behavior change; pure clarity layer.
