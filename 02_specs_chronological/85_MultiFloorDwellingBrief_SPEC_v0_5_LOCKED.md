# MULTI-FLOOR DWELLING BRIEF SPEC v0.5 LOCKED — domain dataclass for multi-floor residential briefs (B-NEW-T3 enabler #1 of 4)

**Component**: NEW domain dataclass (lives at `buildemup/domain/multi_floor_brief.py`).
**Spec status**: **v0.5 LOCKED.** Ramalingam directive: "Do the patches and lock it" (S40-continuation).
**Authority**: Ramalingam directive at S40-continuation: "we will go by path C... I want option b multi floor and let's get the spec locked first." LOCK granted at v0.5 after four critique rounds (v0.1 → v0.2 → v0.3 → v0.4 → v0.5).
...
**Note on this file vs file 84 (PROPOSED)**: file 84 preserves the PROPOSED text for chronological history per Rule 3. File 85 is the LOCKED version with status fields updated. Spec content (schema, invariants, behaviour, test plan) is identical between files.

---

## § 0 — LOCK declaration

**v0.5 LOCKED at S40-continuation per Ramalingam directive ("Do the patches and lock it").**

Per Rule 8 (LOCK authority belongs to Ramalingam alone), this declaration constitutes the LOCK trigger. v0.5 PROPOSED contents are promoted to v0.5 LOCKED with no further patches. The PROPOSED text is preserved chronologically at file 84 per Rule 3.

**Critique walk history**:
- v0.1 PROPOSED → critique walk #1 → v0.2 PROPOSED (CRITICAL: master-floor semantics underpowered, exactly-one-master invariant missing; MAJOR: label normalization absent).
- v0.2 PROPOSED → critique walk #2 → v0.3 PROPOSED (CRITICAL: nested-rewrite identity drift; runtime selector validation missing).
- v0.3 PROPOSED → critique walk #3 → v0.4 PROPOSED (no schema changes; documentation polish).
- v0.4 PROPOSED → critique walk #4 → v0.5 PROPOSED (no schema changes; canonical ownership + replay migration + reading guide).
- v0.5 PROPOSED → **LOCKED**.

**Convergence summary**: schema stabilized by v0.3. v0.4 and v0.5 were pure documentation rounds. The 6 invariants (MFDB-1 through MFDB-6), the schema (3 fields), and the test plan (~45 tests) are unchanged from v0.3 onward.
...
- After LOCK: spec drafting proceeds to **Spec #2 (C9 Amendment v0.8 — `has_master_bedroom: bool = True` flag on `FloorRoomBrief`)**.
- Code work: deferred until all four specs LOCKED. Specs #2, #3, #4 still pending.
- **Convergence note**: v0.3 → v0.4 was the first round with NO schema changes. v0.4 → v0.5 was the second consecutive round with NO schema changes. The spec converged structurally at v0.3; v0.4 and v0.5 added documentation refinements only. LOCK granted at v0.5.

**Spec #1 of 4 — DONE.** Three remaining: C9 Amendment v0.8, `MultiFloorWetZonePlannedCandidate`, C11a Amendment v1.1.
...
**Summary**: foundation of a 4-spec sequence for B-NEW-T3. v0.3 added 2 new deferral items (B-MFDB-K, B-MFDB-L) and repurposed 1 (B-MFDB-I). v0.4 added 1 new deferral item (B-MFDB-M) for strict-mode lookup helpers. v0.5 added 2 more (B-MFDB-N spec restructure, B-MFDB-O canonical_topology_signature). Cumulative deferrals: 12. None blocking LOCK. **Spec LOCKED at v0.5; B-MFDB-A through B-MFDB-O are durable post-LOCK backlog.**

---

**End of MultiFloorDwellingBrief Spec v0.5 LOCKED.**

**Spec #1 of 4 in the B-NEW-T3 sequence — LOCKED.** Next: Spec #2 (C9 Amendment v0.8).
