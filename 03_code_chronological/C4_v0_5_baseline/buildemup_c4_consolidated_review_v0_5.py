# ============================================================================
# BuildemUp C4 (Plot Analysis) — Consolidated Review Bundle
# ============================================================================
#
# SPEC: v0.5 LOCKED (889 lines, see
#       /mnt/user-data/outputs/buildemup_C4_SPEC_v0_5_PROPOSED.md as locked)
# Session: S29
# Status: code shipped, 1389 / 2 skipped / 0 failed in full project run
#
...
# Concatenating means imports between modules look like cross-references,
...
#   2. C4 schema (the type/enum vocabulary)
...
#
# WHAT TO LOOK AT IF YOU ONLY HAVE 5 MINUTES:
#   - components/c04/plot_analysis.py: derive() — the public entry point
#   - components/c04/schema.py: PlotAnalysis dataclass — the output contract
#   - kb/soil_city_defaults.py: the typed-projection adapter (CG-6/7/8 fix)
#
# CG-FIX MAPPING (where each v0.5 SPEC-AMENDMENT lands in the code):
#   CG-5  PlotOrientation full member names — schema.py + wind_direction.py
#   CG-6  SOIL_PROFILES location           — kb/soil_city_defaults.py (NEW)
#   CG-7  Typed kPa + SoilType conversion  — kb/soil_city_defaults.py
#   CG-8  BEARING_CAPACITY_BY_TYPE define  — kb/soil_city_defaults.py
#   CG-9  ResolvedBrief access path        — plot_analysis.py: _extract_brief_inputs
#   CG-10 KB_VERSION already present       — verify_kb_consistency only checks presence
#
# ============================================================================
...
#   2  C4 schema       components/c04/schema.py                                   331    227
#   3  KB layer        kb/wind_direction.py                                        73     51
#   4  KB layer        kb/city_geography.py                                       113     80
#   5  C4 logic        components/c04/sun_path.py                                 111     81
#   6  C4 logic        components/c04/climate_zone.py                              47     34
#   7  C4 logic        components/c04/soil_estimator.py                            57     44
#   8  C4 logic        components/c04/neighbour_context.py                        102     72
#   9  C4 logic        components/c04/plot_analysis.py                            215    160
#  10  C4 package      components/c04/__init__.py                                  50     44
#  11  Test fixtures   tests/validation/_c4_fixtures.py                            81     59
#  12  Tests           tests/validation/test_c4_plot_analysis.py                  343    255
#  13  Tests           tests/validation/test_c4_kb_consistency.py                  35     22
#  14  Tests           tests/validation/test_c4_immutability.py                    50     37
#  15  Tests           tests/validation/test_c4_solar.py                           69     52
#  16  Tests           tests/validation/test_c4_performance.py                     39     28
...
# LOC: 331 total, 227 non-blank/non-comment
# ============================================================================
...
# LOC: 113 total, 80 non-blank/non-comment
# ============================================================================
...
def verify_kb_consistency() -> None:
    ...
    Raises:
        RuntimeError: if any KB is missing a supported city.
...
# LOC: 111 total, 81 non-blank/non-comment
# ============================================================================
...
# LOC: 57 total, 44 non-blank/non-comment
# ============================================================================
...
def estimate_soil(plot: Plot) -> SoilEstimate:
    ...
    confidence = (
        ConfidenceLevel.LOW
        if typical in HIGH_VARIABILITY_SOIL_TYPES
...
# LOC: 102 total, 72 non-blank/non-comment
# ============================================================================
...
# LOC: 215 total, 160 non-blank/non-comment
# ============================================================================
...
_SQM_PER_SQFT_INV = 10.7639   # sqm × 10.7639 = sqft
...
# LOC: 81 total, 59 non-blank/non-comment
# ============================================================================
...
def bangalore_40x60() -> Plot:
    """~40×60 ft Bangalore plot (12.20 × 18.30 m, ~2403 sqft — firmly T2).

    Strict 40×60 ft = 12.192×18.288 m × 10.7639 lands at 2399.998 sqft
    (one float-cancellation tick below the 2400 sqft T1/T2 boundary).
    Fixture nudges to 12.20 × 18.30 to keep the test's intent ("T2")
    independent of float boundary fragility.
    """
    return Plot(
        width_m=12.20, depth_m=18.30, facing=PlotOrientation.EAST,
...
# LOC: 343 total, 255 non-blank/non-comment
# ============================================================================
...
def test_tier_t2_standard_bangalore_40x60():
    """40×60ft fixture (~2403 sqft, see fixture docstring) is T2_STANDARD."""
    pa = derive(make_brief(bangalore_40x60()), now=1.0)
    assert pa.tier == PlotTier.T2_STANDARD
    assert 2400.0 <= pa.area_sqft < 2410.0
...
# LOC: 35 total, 22 non-blank/non-comment
# ============================================================================
...
# LOC: 39 total, 28 non-blank/non-comment
# ============================================================================

"""Tier-1 perf test for C4: derive() under 10ms typical.

Per SPEC v0.5 LOCKED § 7 (item from external critique #13).
Demonstrates the no-caching call holds — C4 is pure-CPU; KBs are loaded
at module import.
"""
from __future__ import annotations
...
def test_derive_completes_under_10ms_for_typical_plot():
    """100-call P95 must beat 10ms per call (very loose; warm-CI safety margin).

    Measures p95 across 100 invocations to mute one-off jitter on the host.
    """
    ...
    samples_ms.sort()
    p95 = samples_ms[94]   # 95th percentile of 100 samples (index 94 = 95th)

    assert p95 < 10.0, (
        f"p95 derive() time = {p95:.3f}ms — exceeds 10ms cap. "
        f"min={samples_ms[0]:.3f}ms, median={samples_ms[49]:.3f}ms, "
        f"max={samples_ms[-1]:.3f}ms"
    )
