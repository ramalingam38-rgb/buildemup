...
__all__ = [
    ...
    "validate_privacy_zoning",  # B-NEW-J (S38)
]


# =============================================================================
# B-NEW-J (S38) — privacy zoning predicate (PRIVATE band not on road wall).
# =============================================================================

# Map each PlotOrientation (the plot's road-facing direction) to the set of
# PlotOrientations a PRIVATE band must NOT occupy.
#
# Cardinal facings forbid only the matching cardinal.
# Intercardinal facings forbid the intercardinal itself and its two adjacent
# cardinals (e.g., NORTHEAST forbids NE, N, and E because all three are
# exposed to the road on a corner-facing plot).
_ROAD_FACING_FORBIDDEN: dict[PlotOrientation, frozenset[PlotOrientation]] = {
    PlotOrientation.NORTH: frozenset({PlotOrientation.NORTH}),
    PlotOrientation.SOUTH: frozenset({PlotOrientation.SOUTH}),
    PlotOrientation.EAST: frozenset({PlotOrientation.EAST}),
    PlotOrientation.WEST: frozenset({PlotOrientation.WEST}),
    PlotOrientation.NORTHEAST: frozenset({
        PlotOrientation.NORTHEAST,
        PlotOrientation.NORTH,
        PlotOrientation.EAST,
    }),
    PlotOrientation.NORTHWEST: frozenset({
        PlotOrientation.NORTHWEST,
        PlotOrientation.NORTH,
        PlotOrientation.WEST,
    }),
    PlotOrientation.SOUTHEAST: frozenset({
        PlotOrientation.SOUTHEAST,
        PlotOrientation.SOUTH,
        PlotOrientation.EAST,
    }),
    PlotOrientation.SOUTHWEST: frozenset({
...
def validate_privacy_zoning(
    ...
    """C5 privacy-zoning predicate. Per B-NEW-J (S38).

    Validates that a TopologyCandidate's PRIVATE band does NOT occupy
    the road-facing direction set derived from `plot_facing`.

    Cardinal facings forbid the single matching cardinal.
    Intercardinal facings forbid the intercardinal itself plus its two
    adjacent cardinals (exposed to the road on a corner-facing plot).

    Vacuous-pass: if `zone_bands` has no PRIVATE entry, returns
    (True, None). The PRIVATE-band-presence rule itself is not in
    scope here.

    C11a Tier A registers this as `C5.privacy_zoning` for M2 (vertical
    flip) and M5 (zone swap) operators.

    Pure function. Does not mutate inputs.

    Args:
        zone_bands: TopologyCandidate.zone_bands mapping.
        plot_facing: PlotOrientation of the plot's road-facing side.
    ...
    if plot_facing not in _ROAD_FACING_FORBIDDEN:
        return (
            False,
            f"C5 privacy_zoning: unrecognised plot_facing="
            f"{plot_facing!r}",
        )
    forbidden = _ROAD_FACING_FORBIDDEN[plot_facing]

    if private_dir in forbidden:
        forbidden_str = ", ".join(sorted(d.value for d in forbidden))
        return (
            False,
            f"C5 privacy_zoning: PRIVATE band at {private_dir.value} "
            f"is in the road-facing forbidden set "
            f"{{{forbidden_str}}} for plot_facing="
            f"{plot_facing.value}",
        )
