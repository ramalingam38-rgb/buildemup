...
# C8 has three standalone error classes (no shared base), each gets its own
# explicit declaration. CorridorTooNarrowError + CorridorSelfIntersectionError
# scope to a single corridor candidate (per_candidate). CorridorDispatchError
# is also per_candidate per its docstring (carries candidate_index).
...
class CorridorTooNarrowError(ValueError):
    ...
    severity_tier: ClassVar[_SeverityTier] = "per_candidate"  # B-NEW-P (S38)
...
class CorridorSelfIntersectionError(RuntimeError):
    ...
    # B-NEW-P (S38): "systemic" — this is a programmer-error / dispatcher bug
    # that shouldn't be aggregated like a per-candidate infeasibility. If it
    # ever fires, the safe behaviour is to halt the batch.
