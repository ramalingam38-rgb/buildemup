"""
================================================================================
C3b v0.6 LOCKED — Post-Layout Trade-off Negotiation (CONSOLIDATED SOURCE)
================================================================================

Spec:   C3b v0.6.LOCKED (S52 Round 2 amendments under Ramalingam delegation)
Build:  S52 — full Phase α/β/γ/δ/ε/ζ + orchestrator +
              SQLite WAL + event log + checkpoint hash chain +
              9 v0.5 amendments (A1–A9) + 4 v0.6 amendments (B1–B4) +
              R17 + R18 + R19 + R20 invariants
Tests:  450 passing (3 skipped), 0 failed

v0.6 amendments integrated:
  B1 — Metadata extension channel (R19 protected — excluded from canonical sig)
  B2 — Advisory lint severity tiers (HARD_BLOCK / REVIEW_NEEDED / WARN)
       with token-aware word-boundary matching
...
New invariants:
  ...
  R20 — event log checkpoint hash chain integrity must verify on load

v0.5 amendments preserved (unchanged at v0.6):
  A1–A9 + R17 + R18
...
Module assembly order (dependency-DAG topological):
  ...
  advisory_lint   → 3-tier severity system + token-aware matching (v0.6 B2)
  contracts       → C3bInputBundle + upstream-stub re-exports
  schema          → 20+ dataclasses with R-invariants (R17/R18/R19 enforced)
                    TopologyInvarianceResult gains graded divergence (v0.6 B3)
                    TradeoffSession gains extension_metadata (v0.6 B1)
...
────────────────────────────────────────────────────────────────────────────────
Public API surface (v0.6 additions marked):

  from c3b_consolidated import (
      # v0.4/v0.5 core (preserved)
      ...
      # v0.6 B2 additions
      LintTier, LintAuditEntry, lint_advisory_text_tiered,
      # v0.6 B4 additions
...
C3B_VERSION:                Final[str] = "v0.6.LOCKED"
C3B_SESSION_SCHEMA_VERSION: Final[int] = 4
...
# ════════════════════════════════════════════════════════════════════════════════
# § END OF C3b v0.6 LOCKED CONSOLIDATED SOURCE
# ════════════════════════════════════════════════════════════════════════════════
# Tests: 450 passing in tests/test_c03b/ (3 skipped, 0 failed).
