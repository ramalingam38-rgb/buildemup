"""
================================================================================
C3b v0.4 LOCKED — Post-Layout Trade-off Negotiation (CONSOLIDATED SOURCE)
================================================================================
...
Spec:   C3b v0.4.LOCKED (S50 Ramalingam adjudication)
Build:  S52 — full Phase α/β/γ/δ/ε/ζ + orchestrator + SQLite WAL
Tests:  382 passing (2 skipped), 0 failed

This file is provided for review / archival / portability. The CANONICAL
source remains the modular tree. To regenerate, re-run the assembly
script at /tmp/build_consolidated.py.
...
Module assembly order (dependency-DAG topological):
  ...
  schema          → 18 dataclasses with R-invariants
  ...
  phases.alpha    → canonicalization + applicability boundary
  phases.beta     → tweak generation per layout
  phases.gamma    → impact verification
  phases.delta    → presentation + signing
  phases.epsilon  → user-turn handling (R14/R15/R16)
...
NOTE: This consolidated file has IMPORT statements that reference
buildemup.components.* paths. To run it standalone, you would need to
either:
  (a) place it inside the buildemup project tree, OR
  (b) stub out the upstream imports (c15.schema, c16.contracts, c10,
      c12, c13, c07, c04, c03a, utils.confidence, utils.transparency)

The intent of this consolidated file is REVIEW + DOCUMENTATION, not
...
C3B_VERSION:                Final[str] = "v0.4.LOCKED"
C3B_SESSION_SCHEMA_VERSION: Final[int] = 2
...
ITERATION_CAP_DEFAULT:      Final[int] = 5
...
Spec: C3b v0.4.LOCKED. Build session: S52.
...
class C3bRuntimeConfig:
    ...
    """STRICT raises per-tweak errors; WARN collects them. LocalTradeoffError
    always halts regardless."""

    iteration_cap: int = ITERATION_CAP_DEFAULT
    """Default 5. Hard ceiling 7 (enforced at construction).
    Phase ε forces finalization when iteration_count >= iteration_cap."""

    enable_q3_level_b_logging: bool = True
    """Q3 Level B logging (inherited from C3a v0.2.1) — every SessionTurn
    records the full presented_tweaks set, not just chosen. Should
    always be True; flag exists for test isolation only."""
...
def compute_canonical_replay_signature(
    *,
    ...
    payload = {
        ...
        "session_history":               _canon(session.session_history),
...
def check_pareto_diversity(bundle: C3bInputBundle) -> Optional[str]:
    ...
    return (
        ...
        "(within 15% on cost and 10% on sqft). C3b's tweak suggestions need "
        "diverse layouts to be useful. You could explore a different brief "
        "configuration or accept one of the layouts as-is."
    )
...
def check_upstream_versions(bundle: C3bInputBundle) -> None:
    """Verify upstream-component versions match EXPECTED_C* pins. Raises
    UpstreamSchemaDriftError on mismatch (per spec § 4.1)."""
    # We can check C15 via problem_reports[0].c15_version
    if bundle.problem_reports:
        observed = bundle.problem_reports[0].c15_version
        if observed != EXPECTED_C15_VERSION:
            raise UpstreamSchemaDriftError(
                f"C15 version drift: observed {observed!r}, expected "
                f"{EXPECTED_C15_VERSION!r}",
                expected_version=EXPECTED_C15_VERSION,
    ...
    if action in ("accepted_tweak", "rejected_tweak"):
        ...
        if action == "accepted_tweak":
            if tweak.severity_tier == "light":
                ...
                new_session, apply_outcome = _apply_medium_tweak_emit_rerun(
                    new_session, tweak, config,
                )
    ...
    if post_status in ("open_for_user_input", "awaiting_subset_rerun"):
        if next_iteration_count >= session.iteration_cap:
            ...
            new_advisory_flags.append(AdvisoryFlag(
                ...
                advisory_note=(
                    "You've explored the typical number of tweak rounds. "
                    "You could finalize a layout now or step back to your "
                    "brief if your scope is changing."
                ),
                related_ids=(),
            ))

    # Append the turn (R12 Q3 Level B logging: presented_tweaks records
    # what was visible to the user this turn)
    turn = SessionTurn(
...
# ════════════════════════════════════════════════════════════════════════════════
# § END OF C3b v0.4 LOCKED CONSOLIDATED SOURCE
# ════════════════════════════════════════════════════════════════════════════════
# Lines: see wc -l output at top of generation.
# Tests: 382 passing in tests/test_c03b/ (2 skipped, 0 failed).
