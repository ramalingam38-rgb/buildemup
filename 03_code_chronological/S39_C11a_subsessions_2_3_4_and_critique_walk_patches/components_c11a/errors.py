...
== Registry mechanism (B-NEW-X, S39 critique walk patch) ==

Subclasses register into a module-level WeakSet at class-definition
time via `__init_subclass__`. The Phase 0 severity-classification
audit walks this registry instead of `cls.__subclasses__()` because:

  1. `__subclasses__()` accumulates ALL definitions for the process
     lifetime (including dynamically-created test fixtures), causing
     audit-output drift between runs in long-running servers.
  2. `__subclasses__()` returns subclasses in non-deterministic order.
  3. WeakSet membership is GC-collectable: dynamically-created
     subclasses (test fixtures with no other refs) drop out
     automatically.

Spec gap: F-v4-5 specified "audit severity_tier on every error class"
but didn't pin the registry mechanism. S39 critique walk surfaced
`__subclasses__()` as fragile; this is the v1 fix.
"""
...
# =============================================================================
# Module-level error registry (B-NEW-X)
# =============================================================================
#
# WeakSet of every concrete TopologyMutationError subclass. Populated
# at class-definition time by `__init_subclass__` on the base. The
# base class itself is included via explicit add() at module load.
#
# Public read accessor: `iter_registered_errors()` — returns subclasses
# in deterministic order (sorted by qualified name).


_ERROR_REGISTRY: "weakref.WeakSet[type]" = weakref.WeakSet()


def iter_registered_errors() -> tuple[type, ...]:
    """Return the registered TopologyMutationError subclasses in
    deterministic order (sorted by `__module__.__qualname__`).

    Used by `validate_severity_classification_audit()` per § 0.5
    Phase 0 audit. The deterministic sort is important: if the audit
    fails, the diagnostic message lists offenders in stable order so
    log output is reproducible across runs.

    WeakSet iteration is intrinsically non-deterministic (set ordering),
    hence the explicit sort here.
    """
    snapshot = list(_ERROR_REGISTRY)
    snapshot.sort(key=lambda c: f"{c.__module__}.{c.__qualname__}")
...
class TopologyMutationError(Exception):
    ...
    All subclasses auto-register into `_ERROR_REGISTRY` via
    `__init_subclass__` (B-NEW-X). The base itself is added at
    module-load time after this class is defined.
    """

    severity_tier: ClassVar[_SeverityTier] = "per_candidate"

    def __init_subclass__(cls, **kwargs: object) -> None:
        """Register every TopologyMutationError subclass into the
        process-wide WeakSet at class-definition time.

        WeakSet semantics ensure dynamically-created subclasses
        (e.g., test fixtures with no module-level references) drop
        out when garbage-collected — preventing the audit-state
        accumulation that `__subclasses__()` traversal exhibits.
        """
        super().__init_subclass__(**kwargs)
        _ERROR_REGISTRY.add(cls)


# Register the base class itself — `__init_subclass__` only fires for
# subclasses, not the base.
_ERROR_REGISTRY.add(TopologyMutationError)
