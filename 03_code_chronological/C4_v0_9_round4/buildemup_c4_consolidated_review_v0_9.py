# ============================================================================
# BuildemUp C4 (Plot Analysis) — Consolidated Review Bundle (v0.9)
# ============================================================================
#
# SPEC: v0.9 LOCKED — patch delta on top of v0.8 LOCKED.
...
#   /mnt/user-data/outputs/buildemup_C4_SPEC_v0_9_LOCKED.md    (this LOCK)
# Session: S29
# Status: 1415 passed / 2 skipped / 0 failed (was 1414/2/0 at v0.8 LOCK)
#
...
# ============================================================================
#
# Each backlog item below is filed in spec § 16 with: ID, description,
# origin (which walk surfaced it), citation (web-verified per Rule 7 where
# applicable), trigger condition, S-scope verdict, and effort estimate.
# Inline review markers also appear at the top of the file(s) each item
# touches, so a critique reviewer can see them in context.
#
# Marker convention: each affected source file has a
#   '# ─── DEFERRED BACKLOG (REVIEW MARKER) ───' block in its docstring.
# The blocks are pure comments — strippable after review-cycle close.
#
...
#
# 7 PUSH-BACKS HELD across S29 critique rounds (see specs v0.6-v0.9 § 15):
#   - v0.6: longitude omission (mathematically irrelevant; documented)
#   - v0.6: trace_id format at C4 (filed as B-078 instead — system-wide policy)
#   - v0.7: city normalization 'silent fail' framing (verify_kb_consistency
#           was already fail-fast; framing was wrong, refactor still done)
#   - v0.7: aspect_ratio width-zero check (Plot.__post_init__ enforces 3-60m)
#   - v0.8: module-load coupling for wind_direction → wind_load (no cycle risk)
#   - v0.8: cross-KB semantic tests (would be tautological or replace cited
#           sources with intuition; source-citation discipline is the guardrail)
#   - v0.8 + v0.9: derive() 'god function' / plugin hooks (YAGNI; 80 LOC linear)
#
# ============================================================================
# WHAT CHANGED IN v0.9 (vs v0.8 bundle)
# ============================================================================
# 1 SPEC-AMENDMENT, 0 doc-only, 1 test refinement.
# Plus: Rule 7 amendment (web-search now MANDATORY on every critique walk).
#
#  Walk #  Fix                                                  Files touched
#  ------  ---------------------------------------------------  ---------------------
#    #1    [PUSH BACK — fail-fast IS the design]                (no change)
#    #2    [BACKLOG only — B-082 structured kPa error fields]   (no change)
#    #3    [BACKLOG only — B-083 sub-city soil zoning]          (no change)
#    #4    [DUPLICATE — already B-070]                          (no change)
#    #5    [DUPLICATE — already B-076]                          (no change)
#    #6    [DUPLICATE — already B-081]                          (no change)
#    #7    Exact 1/SQM_PER_SQFT inverse for area_sqft display   plot_analysis.py
#    #8    [PUSH BACK — YAGNI plugin architecture]              (no change)
#
# v0.8 → v0.9 SOURCE DELTA AT A GLANCE
#   plot_analysis.py:          ~5 LOC   (10.7639 → exact 1/SQM_PER_SQFT inverse)
#   test_c4_plot_analysis.py:  +14 LOC  (1 new boundary test, 2400.0 exact)
#   test_c4_property_based.py: ±0 LOC   (round-trip uses exact factor; tighter tol)
#   + inline backlog markers in 6 source files (~80 LOC of comments, strippable)
#
# CUMULATIVE LINEAGE (v0.5 → v0.9): C4 has been through 5 LOCK rounds in S29.
#   v0.5: initial build (10 source + 7 test files; CG-5..10 build-grep fixes)
#   v0.6: 5 SPEC-AMENDMENTS; 5 backlog (B-074..B-078)
#   v0.7: 4 SPEC-AMENDMENTS; 1 backlog (B-079)
#   v0.8: 3 SPEC-AMENDMENTS; 2 backlog (B-080, B-081)
#   v0.9: 1 SPEC-AMENDMENT; 2 backlog (B-082, B-083); Rule 7 amended
#   Total: 13 SPEC-AMENDMENTS, 10 new backlog items, 7 push-backs documented
#
# WHERE TO LOOK IF YOU ONLY HAVE 5 MINUTES
# ----------------------------------------
#   - components/c04/plot_analysis.py: SQFT_PER_SQM exact-inverse (v0.9 fix)
#   - components/c04/plot_analysis.py docstring: ALL orchestrator-level backlogs
#   - components/c04/soil_estimator.py docstring: soil-related backlogs (B-074/077/082)
#   - kb/soil_city_defaults.py docstring: KB-layer backlogs (B-072/079/083)
#
...
#   4  KB layer        kb/city_geography.py                                       179    132
...
#  12  Tests           tests/validation/test_c4_plot_analysis.py                  587    439
#  13  Tests           tests/validation/test_c4_kb_consistency.py                  99     66
...
# LOC: 179 total, 132 non-blank/non-comment
# ============================================================================
...
# LOC: 587 total, 439 non-blank/non-comment
# ============================================================================
...
def test_area_sqft_display_matches_textbook_at_40x60ft_boundary():
    ...
    pa = derive(make_brief(plot), now=1.0)
    # Tight equality: the exact inverse means no rounding drift.
...
# LOC: 99 total, 66 non-blank/non-comment
# ============================================================================
