# ============================================================================
# BuildemUp C4 (Plot Analysis) — Consolidated Review Bundle (v0.6)
# ============================================================================
#
# SPEC: v0.6 LOCKED — patch delta on top of v0.5 LOCKED. Both specs at:
...
# Session: S29
# Status: code shipped, 1402 / 2 skipped / 0 failed in full project run
#         (was 1389/2/0 at v0.5 LOCK — net +13 from v0.6 patches)
#
...
#
# WHAT CHANGED IN v0.6 (vs v0.5 bundle)
# -------------------------------------
# 5 SPEC-AMENDMENTS landed in code, 1 doc-only, 1 perf-test re-cap.
# All field additions are safe-defaulted — zero contract-breaking changes.
#
#  Walk #  Fix                                                  Files touched
#  ------  ---------------------------------------------------  --------------------
#    #1    Tier classification → sqm space (exact 0.09290304)   plot_analysis.py
#    #2/7  SoilEstimate.provenance_note (b074 breadcrumb)       schema.py, soil_estimator.py
#    #3    WindContext.confidence (MEDIUM until B-070)          schema.py, wind_direction.py
#    #4    [BACKLOG only — B-075 aspect classification]         (none — see B-075)
#    #5    NeighbourContext.corner_assumption (b076 breadcrumb) schema.py, neighbour_context.py
#    #6    KB sanity-range checks (wind/lat/kPa)                city_geography.py
#    #8    [DOCUMENTED] longitude omission rationale            sun_path.py docstring
#    #9    [PUSH-BACK + B-078] trace_id format policy           (no code change)
#    #10   Perf cap 10ms → 1ms; samples 100 → 1000              test_c4_performance.py
#
# 5 NEW BACKLOG ITEMS filed at PROPOSED time (Rule 9.2):
#   B-074  MEDIUM_ROCK proper kPa value (currently underestimated to 660 kPa
#          per IS 6403 typical ~1000-1500 kPa for medium rock)
#   B-075  AspectClass enum + extreme-plot detection
#   B-076  Plot.corner_orientation input field (touches C1/C3a/C7)
#   B-077  Richer multi-state soil confidence model
#   B-078  System-wide trace_id lifecycle policy
#
# WHERE TO LOOK IF YOU ONLY HAVE 5 MINUTES
# ----------------------------------------
#   - components/c04/schema.py: the 3 v0.6 field additions
#     (WindContext.confidence, SoilEstimate.provenance_note,
#      NeighbourContext.corner_assumption — all safe-defaulted)
#   - components/c04/plot_analysis.py: SQM_PER_SQFT + _classify_tier (sqm space)
#   - components/c04/soil_estimator.py: _APPROXIMATED_USER_INPUT_NOTES table
#   - kb/city_geography.py: verify_kb_consistency value-range checks
#
...
#
# v0.5 → v0.6 SOURCE DELTA AT A GLANCE
# (lines = total file LOC; the 'delta' column shows v0.6 file size minus v0.5)
# Schema:                   +66 LOC  (3 field additions + docstrings)
# wind_direction.py:        +6  LOC  (confidence kwarg on 6 entries)
# city_geography.py:        +29 LOC  (3 sanity-range loops + docstring)
# sun_path.py:              +12 LOC  (longitude omission rationale, doc only)
# soil_estimator.py:        +20 LOC  (note table + populate logic)
# neighbour_context.py:     +12 LOC  (corner_assumption tracking)
# plot_analysis.py:         +28 LOC  (sqm thresholds + sqm-space _classify_tier)
# tests:                    +13 NEW (10 in main, 3 in kb_consistency)
#
# ============================================================================


# ============================================================================
# TABLE OF CONTENTS  (v0.6 bundle)
# ============================================================================
#
#   #  CATEGORY        PATH                                                     TOTAL   CODE
#
#   1  KB layer        kb/soil_city_defaults.py                                   156     76
#   2  C4 schema       components/c04/schema.py                                   367    257
#   3  KB layer        kb/wind_direction.py                                        87     57
#   4  KB layer        kb/city_geography.py                                       147    108
#   5  C4 logic        components/c04/sun_path.py                                 123     92
#   6  C4 logic        components/c04/climate_zone.py                              47     34
#   7  C4 logic        components/c04/soil_estimator.py                            89     60
#   8  C4 logic        components/c04/neighbour_context.py                        113     76
#   9  C4 logic        components/c04/plot_analysis.py                            235    171
#  10  C4 package      components/c04/__init__.py                                  50     44
#  11  Test fixtures   tests/validation/_c4_fixtures.py                            84     62
#  12  Tests           tests/validation/test_c4_plot_analysis.py                  467    352
...
#  16  Tests           tests/validation/test_c4_performance.py                     46     33
...
# LOC: 367 total, 257 non-blank/non-comment
# ============================================================================
...
class WindContext:
    ...
    `basic_speed_ms` is a property reading from kb/wind_load at access
    time so we don't duplicate IS-875 values. `city` is stored only as
    a lookup key for that property.
    ...
    city: str                              # lookup key for basic_speed_ms
    confidence: ConfidenceLevel = ConfidenceLevel.MEDIUM   # NEW v0.6 (walk #3)

    @property
    def basic_speed_ms(self) -> float:
        """IS 875 Part 3 basic wind speed for this city, in m/s.

        Reads from kb/wind_load at access time — no duplication.
        """
        # Local import to avoid module-load order issues.
...
# LOC: 87 total, 57 non-blank/non-comment
# ============================================================================
...
# LOC: 47 total, 34 non-blank/non-comment
# ============================================================================

"""
BuildemUp† — C4 climate zone lookup.

Thin wrapper around kb/city_geography.CITY_GEOGRAPHY.
...
def lookup_climate_zone(city: str) -> ClimateZone:
    """Look up climate zone for a (normalized) city.

    Caller must pass a normalized city (lowercase, stripped). The Plot
    constructor already does this, but C4 also defensively normalizes
    on entry per spec § 2.

    Raises:
        ValueError: if city is not in CITY_GEOGRAPHY.
    """
    if city not in CITY_GEOGRAPHY:
        raise ValueError(
            f"city '{city}' missing from CITY_GEOGRAPHY; check "
            f"kb/city_geography.py against domain.plot.SUPPORTED_CITIES"
        )
    return CITY_GEOGRAPHY[city].climate_zone


def lookup_latitude(city: str) -> float:
    """Look up latitude (degrees) for a (normalized) city.

    Same contract as lookup_climate_zone.
    """
    if city not in CITY_GEOGRAPHY:
        raise ValueError(
            f"city '{city}' missing from CITY_GEOGRAPHY; check "
    ...
    return CITY_GEOGRAPHY[city].latitude_deg


__all__ = ["lookup_climate_zone", "lookup_latitude"]
...
# LOC: 235 total, 171 non-blank/non-comment
# ============================================================================
...
from buildemup.components.c04.climate_zone     import lookup_climate_zone, lookup_latitude
...
from buildemup.kb.city_geography               import CITY_GEOGRAPHY, kb_versions
...
def derive(brief: object, *, now: float) -> PlotAnalysis:
    ...
    # Normalize city defensively (Plot already does this, but C4 belt + braces
    # in case future callers bypass Plot.__post_init__ via construction).
    city = plot.city.strip().lower()
    if city not in CITY_GEOGRAPHY:
        raise ValueError(
            f"city '{city}' missing from CITY_GEOGRAPHY; check "
            f"kb/city_geography.py against domain.plot.SUPPORTED_CITIES"
...
# LOC: 467 total, 352 non-blank/non-comment
# ============================================================================
...
# LOC: 46 total, 33 non-blank/non-comment
# ============================================================================

"""Tier-1 perf test for C4: derive() under 1ms typical (v0.6 walk #10).

Per SPEC v0.6 LOCKED § 14.7. Tightened from v0.5's 10ms cap based on
S29 measured baseline:

  1000-call warmed measurement on dev host:
    min:   0.009ms     p50:   0.010ms     p95:   0.015ms
    p99:   0.031ms     max:   0.064ms     mean:  0.011ms

  v0.6 cap = 1.0 ms = ~66× current p95 — comfortable headroom for slower
  CI hosts while catching any meaningful regression (e.g., accidental
  I/O at call time, or O(n²) loops added in future).
"""
from __future__ import annotations
...
def test_derive_completes_under_1ms_for_typical_plot():
    ...
    assert p95 < 1.0, (
        f"p95 derive() time = {p95:.3f}ms — exceeds 1.0ms cap. "
