# ============================================================================
# BuildemUp C4 (Plot Analysis) — Consolidated Review Bundle (v0.8)
# ============================================================================
#
# SPEC: v0.8 LOCKED — patch delta on top of v0.7 LOCKED. Specs at:
...
#   /mnt/user-data/outputs/buildemup_C4_SPEC_v0_8_LOCKED.md    (this LOCK)
# Session: S29
# Status: code shipped, 1414 / 2 skipped / 0 failed in full project run
#         (was 1408/2/0 at v0.7 LOCK — net +6 from v0.8 patches)
#
...
#
# WHAT CHANGED IN v0.8 (vs v0.7 bundle)
# -------------------------------------
# 3 SPEC-AMENDMENTS, 0 doc-only changes, 0 test reorganization.
# All changes are internal — zero contract-breaking changes.
#
#  Walk #  Fix                                                  Files touched
#  ------  ---------------------------------------------------  ----------------------
#    #1    [PUSH BACK — module-load coupling not a real risk]   (no change)
#    #2    [BACKLOG only — B-080 dynamic KB mutation]            (no change)
#    #3    [DUPLICATE of B-079 already filed]                    (no change)
#    #4    [DUPLICATE of v0.7 §14.3 docstring already in place]  (no change)
#    #5    KB key normalization enforced at module load          city_geography.py
#    #6    [BACKLOG only — B-081 rolling baseline / CI infra]    (no change)
#    #7    WindContext.city field REMOVED (dead metadata)        schema.py, wind_direction.py
#    #8    Approximation notes → KB layer (data, not logic)      soil_city_defaults.py, soil_estimator.py
#    #9    [PUSH BACK — semantic cross-KB tests not coherent]    (no change)
#   #10    [PUSH BACK — derive() is 80 LOC linear, not 'god']    (no change)
#
# 2 NEW BACKLOG ITEMS filed at PROPOSED time (Rule 9.2):
#   B-080  Dynamic KB mutation: re-validation if KBs are mutated outside
#          module-load. Trigger: hot reload, admin tools, runtime monkey-patch.
#   B-081  Performance test: rolling baseline OR env-configurable threshold.
#          Trigger: CI infrastructure live (GHA, dedicated runner, etc.).
#
# 3 PUSHBACKS HELD (no code change):
#   #1   wind_load is a leaf KB (stdlib only); circular-import risk requires
#        deliberate code change, not 'accidental'. Previous round (v0.6 walk #2)
#        flagged the OPPOSITE design — both have tradeoffs; we're done oscillating.
#   #9   Each KB cites authoritative source (NBC / IS-875 / IMD / IS-6403).
#        Cross-KB 'coherence' tests would either be tautological or replace
#        cited sources with expert intuition. Source-citation discipline at
#        the KB level is the right guardrail and is in place.
#  #10   derive() is 80 LOC linear (36 docstring + ~50 body). Each step is
#        already in its own module. 4 sub-pipelines would create files with
#        one function call each. SRP is about cohesion, not LOC.
#
# 2 DUPLICATES NOTED:
#   #3   already B-079 (filed in v0.7).
#   #4   already DOCUMENTED in v0.7 §14.3 derive() docstring.
#
# WHERE TO LOOK IF YOU ONLY HAVE 5 MINUTES
# ----------------------------------------
#   - components/c04/schema.py: WindContext now has 4 fields (no city)
#   - kb/wind_direction.py: PREVAILING_WIND entries no longer carry city kwarg
#                            (the dict KEY is the city)
#   - kb/soil_city_defaults.py: NEW APPROXIMATION_NOTES_BY_SOIL_TYPE export
#   - components/c04/soil_estimator.py: imports the table from the KB layer
#   - kb/city_geography.py: verify_kb_consistency now also enforces normalize_city(key)==key
#
...
#
# v0.7 → v0.8 SOURCE DELTA AT A GLANCE
#   schema.py:                 -1 LOC  (city field removed; docstring updated)
#   wind_direction.py:         -6 LOC  (city kwargs removed; docstring updated)
#   soil_city_defaults.py:     +25 LOC (APPROXIMATION_NOTES table + docs)
#   soil_estimator.py:         -22 LOC (local table removed; import from KB)
#   city_geography.py:         +24 LOC (KB key normalization enforcement)
#   test_c4_plot_analysis.py:  +33 LOC (4 new tests for v0.8 amendments)
#   test_c4_kb_consistency.py: +24 LOC (2 new key-normalization drift tests)
#
# CUMULATIVE LINEAGE (v0.5 → v0.8): C4 has been through 4 LOCK rounds in S29.
#   v0.5: initial build (10 source + 7 test files; CG-5..10 fixes from build-grep)
#   v0.6: 5 SPEC-AMENDMENTS (sqm-space tier, soil/wind/corner provenance, KB
#         sanity ranges, perf cap 10ms→1ms); 5 backlog (B-074..B-078)
#   v0.7: 4 SPEC-AMENDMENTS (centralized validate_city, eager basic_speed_ms,
#         upstream-invariants doc, perf cap 1ms→3ms); 1 backlog (B-079)
#   v0.8: 3 SPEC-AMENDMENTS (KB key normalization, drop WindContext.city,
#         move soil approximation table to KB); 2 backlog (B-080, B-081)
#   Total: 12 SPEC-AMENDMENTS, 8 new backlog items, 7 push-backs documented
#
# ============================================================================


# ============================================================================
# TABLE OF CONTENTS  (v0.8 bundle)
# ============================================================================
#
#   #  CATEGORY        PATH                                                     TOTAL   CODE
#
#   1  KB layer        kb/soil_city_defaults.py                                   185     82
#   2  C4 schema       components/c04/schema.py                                   371    262
#   3  KB layer        kb/wind_direction.py                                        95     58
...
#   7  C4 logic        components/c04/soil_estimator.py                            76     61
...
#  12  Tests           tests/validation/test_c4_plot_analysis.py                  570    425
...
# LOC: 185 total, 82 non-blank/non-comment
# ============================================================================
...
# LOC: 95 total, 58 non-blank/non-comment
# ============================================================================
...
# LOC: 76 total, 61 non-blank/non-comment
# ============================================================================
...
# LOC: 570 total, 425 non-blank/non-comment
# ============================================================================
