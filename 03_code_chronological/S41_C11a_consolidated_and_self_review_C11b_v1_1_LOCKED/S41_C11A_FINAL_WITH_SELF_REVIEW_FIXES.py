"""
================================================================================
BuildemUp — C11a B-NEW-T3 + SELF-REVIEW FIXES — FINAL Consolidated Bundle
================================================================================

This is the SINGLE-FILE CONSOLIDATED VIEW of the C11a B-NEW-T3 build INCLUDING
all 8 fixes from the Rule 11 self-review applied at the close of S41.

Supersedes both:
  - S41_B_NEW_T3_CONSOLIDATED_REVIEW.py (initial Sub-3 deliverable; missing wiring)
  - S41_C11A_FINAL_CONSOLIDATED.py (post-wiring; missing self-review fixes)

================================================================================
Rule 11 (newly added this session) and what triggered it
================================================================================

Ramalingam mandated, post-self-analysis: "all spec docs and codes should be
subject to vigorous self analysis and also web research when I give the
critique analysis or after every time the files are created or amended."

Memory record (edit #13):
"BuildemUp Rule 11 — Mandatory vigorous self-analysis + web research on every
spec/code creation, amendment, AND critique walk. Self-analysis: lead with
worst issues; hunt functional bugs, dropped output (Pattern B), unstated
assumptions, fixture brittleness, documented-but-unfixed limits, dead code,
test blind spots. Bar: 'next consumer can use output' not 'tests pass'. Web
research: 1+ search verifying claims. Origin S41: caught apply_m8_real
discarding new wrapper."

================================================================================
8 self-review issues + their fixes (all shipped + tested)
================================================================================

  ISSUE 1 (CRITICAL Pattern B — apply_m8_real discarded new_wrapper):
    FIX: Added MutationApplicationResult.output_candidate field
         (Optional[Any]=None, compare=False, hash=False).
         apply_m8_real now returns new_wrapper on it.
         _tier_b_base.apply_tier_b_operator now returns pipe_result
         .regenerated_candidate on it (closes pre-existing Pattern B
         for M6/M7 Tier B operators that was masked until now).
         M0_BASE carries source as identity output_candidate (single +
         multi-floor paths consistent).

  ISSUE 2 (dead enabled_set_tuple variable):
    FIX: Removed.

  ISSUE 3 (fixture-coincidence floors[0] CDC):
    FIX: Adapter runner signature now
         (per_floor_brief, source_floor_wzpc) -> WZPC.
         Each cascade extracts CDC from THAT floor's own ancestry.
         _dispatch_m8_real no longer reaches into floors[0].

  ISSUE 4 (C9 vs C10 failure conflation):
    FIX: Adapter attaches `.c11a_stage` ("c9"|"c10") to raised
         exceptions. apply_m8_real reads it and routes to the correct
         invalidity_reason: "c9_generation_failed" or
         "c10_validation_failed". Bare exceptions default to c9.

  ISSUE 5 (inline `import dataclasses` x2 in orchestrator):
    FIX: Hoisted to module top with import functools (also needed for
         Issue 6 fix). Inline statements stripped.

  ISSUE 6 (_operator_index_in_family recomputes sort every call):
    FIX: @functools.lru_cache(maxsize=None).

  ISSUE 7 (asymmetric error handling unsurfaced in code):
    FIX: Added explicit code comment in _generate_seeds_multi_floor
         explaining the systemic-severity propagation choice per
         Spec § 3.1.

  ISSUE 8 (test blind spot — no test asserts output_candidate content):
    FIX: New file test_s41_self_review_fixes.py with 10 tests covering
         output_candidate populated, master_bedroom swapped on output,
         output_candidate None on failure, orchestrator end-to-end
         carries mutated wrapper, Tier A predicate-only ops have None,
         runner signature is new shape, c10 classifies distinctly,
         bare exception defaults to c9, M0 carries source single-floor,
         M0 carries source multi-floor.

================================================================================
Test count progression
================================================================================

  Baseline (S39 close, post bundle-merge):       2755 passed / 3 skip
  End of S41 Sub-1 (domain types + Spec #2 v0.12): 2843 (+88)
  End of S41 Sub-2 (non-orchestrator MF support):  2856 (+13)
  End of S41 Sub-3 (helpers + property + integ):   2890 (+34)
  End of S41 wiring (orchestrator MF dispatch):    2899 (+9)
  End of S41 SELF-REVIEW FIXES (this delivery):    2909 (+10)

  Net S41 tests added: 154. Net regressions across S41: 0.

================================================================================
Module sections in this file
================================================================================
  ...
  PART 2 — C9 AMENDMENT (Spec #2 v0.12) — DELTAS
    2.1  components/c09/room_sizer.py
    2.2  components/c09/validator.py

  PART 3 — C11a SPEC #4 v1.6 + SELF-REVIEW FIXES — DELTAS + NEW FILES
    3.1  components/c11a/errors.py — Orchestration error hierarchy
    3.2  components/c11a/schema.py — adds:
         - MutationApplicationResult.floor_label_affected (Spec #4)
         - MutationApplicationResult.output_candidate (FIX 1)
         - TopologyMutationConfig.generation (Spec #4)
         - FloorImpact + MULTI_FLOOR_INVALIDITY_REASONS (Spec #4)
    3.3  components/c11a/cache.py — version bump
    3.4  components/c11a/candidate_context.py — is_real_multi_floor_candidate
    3.5  components/c11a/source_signature.py — multi-floor signature dispatch
    3.6  components/c11a/family_slot_allocator.py — multi_floor_family_id +
         floor_count_for_cap parameter
    3.7  components/c11a/lineage.py — floor_label_affected field
    3.8  components/c11a/orchestrator.py — multi-floor dispatch + Sub-2
         helpers + FIX 5 (module imports), FIX 6 (lru_cache), FIX 7 (asymmetry
         comment), FIX 2 (dead variable removed)
    3.9  components/c11a/m8_floor_swap_real.py — NEW + FIX 1 (output_candidate),
         FIX 3 (per-floor source arg), FIX 4 (c11a_stage classification)
    3.10 components/c11a/multi_floor_c9_c10_adapter.py — NEW + FIX 3 (per-floor
         CDC extraction), FIX 4 (c11a_stage attachment)
    3.11 components/c11a/operators/_tier_b_base.py — FIX 1 (Tier B output_candidate)
    3.12 components/c11a/operators/_base.py — FIX 1 (build_valid_result accepts
         optional output_candidate)
    3.13 components/c11a/operators/m0_base.py — FIX 1 (M0 carries source)

  PART 4 — TESTS
    4.1  tests/test_domain_multi_floor_brief.py — 45 tests (Sub-1)
    4.2  tests/test_c09_v0_8_master_bedroom_flag.py — 8 tests (Sub-1)
    4.3  tests/test_domain_multi_floor_candidate.py — 35 tests (Sub-1)
    4.4  tests/test_c11a/test_c11a_sub2_multi_floor_support.py — 13 tests (Sub-2)
    4.5  tests/_multi_floor_fixtures.py — fixture helpers
    4.6  tests/test_c11a/test_subsession7_multi_floor.py — 21 tests (Sub-3)
    4.7  tests/test_c11a/test_subsession7_multi_floor_properties.py — 9 tests
    4.8  tests/test_integration/test_b_new_t3_pipeline.py — 4 tests
    4.9  tests/test_c11a/test_subsession8_multi_floor_orchestrator.py — 9 tests
    4.10 tests/test_c11a/test_s41_self_review_fixes.py — NEW 10 tests (FIX 8)
    4.11 tests/test_c11a/test_c11a_subsession1_schema.py — DELTA partition sentinel
    4.12 tests/test_c11a/test_c11a_subsession5_cache_versioning.py — DELTA v1.3.0

  PART 5 — SPEC AMENDMENT
    5.1  spec_amendments/91_C9_AMENDMENT_v0_12_LOCKED.md
"""

# This file is a CONCATENATED VIEW — do NOT try to import it.
...
################################################################################
# 2.1  components/c09/room_sizer.py — DELTA
################################################################################
...
################################################################################
# 2.2  components/c09/validator.py — DELTA
################################################################################
...
################################################################################
# PART 3 — C11a SPEC #4 v1.6 + SELF-REVIEW FIXES
################################################################################



################################################################################
# 3.1  components/c11a/errors.py — DELTA
################################################################################
...
################################################################################
# 3.2  components/c11a/schema.py — DELTAS (FloorImpact + output_candidate + generation + invalidity reasons)
################################################################################

# --- schema.py — MutationApplicationResult (gains floor_label_affected + output_candidate FIX 1) ---
...
# --- schema.py — TopologyMutationConfig (gains generation field) ---
...
# --- schema.py — appended FloorImpact + MULTI_FLOOR_INVALIDITY_REASONS ---
# =============================================================================
# Spec #4 v1.6 — FloorImpact (multi-floor pipeline rework, B-NEW-T3 #4)
# =============================================================================
#
...
################################################################################
# 3.3  components/c11a/cache.py — DELTA
################################################################################

# --- cache.py one-line change (v1.0.0 -> v1.3.0) ---
C11A_CACHE_KEY_VERSION: str = "v1.3.0"


################################################################################
# 3.4  components/c11a/candidate_context.py — DELTA
################################################################################
...
################################################################################
# 3.5  components/c11a/source_signature.py — DELTA
################################################################################

# --- source_signature.py (modified derive_canonical_signature + new _derive_multi_floor) ---
...
################################################################################
# 3.6  components/c11a/family_slot_allocator.py — DELTAS
################################################################################

# --- family_slot_allocator.py — allocate_family_slots gains floor_count_for_cap ---
...
# --- family_slot_allocator.py — appended multi_floor_family_id ---
# =============================================================================
...
################################################################################
# 3.7  components/c11a/lineage.py — DELTA
################################################################################

# --- lineage.py — LineageClassification gains floor_label_affected ---
@dataclass(frozen=True)
...
################################################################################
# 3.8  components/c11a/orchestrator.py — DELTAS (multi-floor branch + helpers + FIXES 2/5/6/7)
################################################################################

# --- orchestrator.py — module imports (FIX 5: dataclasses + functools hoisted) ---
from __future__ import annotations
...
# --- orchestrator.py — _generate_seeds_for_one_source (multi-floor branch detection + single-floor legacy path) ---
...
# --- orchestrator.py — _generate_seeds_multi_floor + _dispatch_per_floor + _dispatch_m8_real + _operator_index_in_family (FIX 6 cached) ---
# =============================================================================
...
# --- orchestrator.py — module-level helpers (pre-flights + bipartite + affected_floor_set) ---
# =============================================================================
...
################################################################################
# 3.9  components/c11a/m8_floor_swap_real.py — NEW + FIXES 1/3/4
################################################################################
...
################################################################################
# 3.10 components/c11a/multi_floor_c9_c10_adapter.py — NEW + FIXES 3/4
################################################################################
...
################################################################################
# 3.11 components/c11a/operators/_tier_b_base.py — DELTA (FIX 1 Tier B output_candidate)
################################################################################

# --- _tier_b_base.py — apply_tier_b_operator success path now carries output_candidate ---
...
################################################################################
# 3.12 components/c11a/operators/_base.py — DELTA (FIX 1 build_valid_result output_candidate kwarg)
################################################################################

# --- _base.py — build_valid_result accepts output_candidate kwarg (FIX 1) ---
...
################################################################################
# 3.13 components/c11a/operators/m0_base.py — DELTA (FIX 1 M0 carries source)
################################################################################

# --- m0_base.py — M0 carries source as output_candidate (FIX 1) ---
...
################################################################################
# 4.9  tests/test_c11a/test_subsession8_multi_floor_orchestrator.py
################################################################################
