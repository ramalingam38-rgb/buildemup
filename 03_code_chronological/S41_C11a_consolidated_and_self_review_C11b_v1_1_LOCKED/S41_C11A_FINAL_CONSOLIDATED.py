"""
================================================================================
BuildemUp — C11a B-NEW-T3 FINAL Consolidated Bundle for Review (Session S41)
================================================================================

This is the SINGLE-FILE CONSOLIDATED VIEW of the FULL C11a B-NEW-T3 build,
including the orchestrator wiring + M8 stub-to-real swap + C9->C10 cascade
adapter + lineage threading + integration tests added in this session
following Ramalingam's directive "Yes complete c11a build fully and then
give me the consolidated single file."

Supersedes the earlier S41 consolidated file
(S41_B_NEW_T3_CONSOLIDATED_REVIEW.py) which contained the helpers but
NOT the orchestrator wiring. This file contains everything.

================================================================================
What is fully built in C11a as of this delivery
================================================================================

  ✓ Item 1 — `mutate_topologies` per-floor dispatch wiring (§ 3.2, § 3.3, § 3.10)
  ✓ Item 2 — M8 stub-to-real swap routed via `_dispatch_m8_real` -> apply_m8_real
  ✓ Item 3 — Real C9->C10 cascade adapter (`multi_floor_c9_c10_adapter.py`)
  ✓ Item 4 — Generation counter on TopologyMutationConfig (§ 3.4.1)
  ✓ Item 5 — Multi-floor lineage `floor_label_affected` recording (§ 3.8)
  ✓ Item 6 — Multi-floor signature wired into cache key (§ 3.5 + § 3.6)
  ✓ Item 7 — C11a cross-spec integration tests (orchestrator-routed, § 5.1)

Items 1-7 above are the previously-pending list from the close of S41
Sub-3. All shipped now.

Backlog items (B-C11A-5 through B-C11A-17, plus B-C9-H) remain post-v1
work, gated on real triggers (NSGA-II scoring landing, cross-floor
constraints, etc.). NOT pending C11a build work.

================================================================================
Test count progression across S41
================================================================================

  Baseline (S39 close, post-bundle-merge):              2755 passed / 3 skip
  End of S41 Sub-1 (domain types + Spec #2 v0.12):      2843 passed / 3 skip
  End of S41 Sub-2 (non-orchestrator multi-floor):      2856 passed / 3 skip
  End of S41 Sub-3 (helpers + property + integration):  2890 passed / 3 skip
  End of S41 TODAY (orchestrator wiring + M8 real):     2899 passed / 3 skip
                                                        +9 from orchestrator integration

  Net new tests in S41: 144 (88 + 13 + 34 + 9)
...
================================================================================
  ...
  PART 2 — C9 AMENDMENT (Specs #2 + #2 v0.12) — DELTAS
    2.1  components/c09/room_sizer.py
    2.2  components/c09/validator.py

  PART 3 — C11a SPEC #4 v1.6 — DELTAS + NEW FILES
    3.1  components/c11a/errors.py
    3.2  components/c11a/schema.py
    3.3  components/c11a/cache.py
    ...
    3.7  components/c11a/lineage.py
    3.8  components/c11a/orchestrator.py — includes ALL multi-floor wiring
    3.9  components/c11a/m8_floor_swap_real.py — NEW
    3.10 components/c11a/multi_floor_c9_c10_adapter.py — NEW
  ...
  PART 4 — TESTS
    ...
    4.9  tests/test_c11a/test_subsession8_multi_floor_orchestrator.py — NEW THIS PASS
    4.10 tests/test_c11a/test_c11a_subsession1_schema.py — DELTA (partition sentinel)
...
################################################################################
# 3.2  components/c11a/schema.py — DELTAS (FloorImpact + invalidity reasons + MutationApplicationResult.floor_label_affected + TopologyMutationConfig.generation)
################################################################################

# --- schema.py — MutationApplicationResult (gains floor_label_affected field) ---
...
# --- schema.py — TopologyMutationConfig (gains generation field, cache_relevant=False) ---
...
################################################################################
# 3.6  components/c11a/family_slot_allocator.py — DELTAS (multi_floor_family_id + allocate_family_slots floor_count_for_cap)
################################################################################

# --- family_slot_allocator.py — allocate_family_slots gains floor_count_for_cap kwarg ---
...
################################################################################
# 3.8  components/c11a/orchestrator.py — DELTAS (multi-floor branch + dispatch helpers + 4 module-level helpers from Sub-2)
################################################################################

# --- orchestrator.py — _generate_seeds_for_one_source (with multi-floor detection + dispatch) ---
...
# --- orchestrator.py — _generate_seeds_multi_floor + _dispatch_per_floor_operator + _dispatch_m8_real + _operator_index_in_family ---
# =============================================================================
...
def _generate_seeds_multi_floor(
    *,
    ...
    enabled_set = set(config.enabled_operators)
    enabled_set_tuple = tuple(config.enabled_operators)
...
def _dispatch_m8_real(
    *,
    ...
    # Build the per-floor C9->C10 runner. M8's cascade needs corridor-
    # designed candidates as the C9 input; the source wrapper's first
    # floor's WZPC carries an ancestry chain that includes the CDC tuple
    # via .room_sized_candidate.corridor_designed_candidate. For v1 we
    # use a single-element tuple containing that ancestry CDC.
    try:
        # The source wrapper's first floor's WZPC has an ancestry chain;
        # we use ITS corridor_designed_candidate as the C9 input batch.
        # All floors share grid + plot_analysis (per-floor grid is
        # B-MFDB-C). Single-CDC is sufficient for the v1 cascade — M8
        # produces one master cascade per affected floor.
        first_floor_wzpc = source.floors[0]
        cdc = (
            first_floor_wzpc.room_sized_candidate
            .corridor_designed_candidate
        )
        runner = make_per_floor_c9_runner(
            corridor_designed_candidates=(cdc,),
            grid=grid,
            plot_analysis=plot_analysis,
        )
    except AttributeError as e:
        # Source ancestry chain broken — surface as orchestration drift.
        return MutationApplicationResult(
            operator=operator,
            valid=False,
            invalidity_reason=(
                f"orchestration_state_drift:mfwzp4 — "
                f"source ancestry broken: {e}"
            ),
            topology_variant_id=None,
            rejection_invariant_id=None,
            source_family_id=source_family_id,
            ...
            upstream_regeneration_delta=(),
            floor_label_affected=None,    # M8 is dwelling-wide
        )
...
def _operator_index_in_family(operator: MutationOperator) -> int:
    ...
    # Lex-ASC across all operators with this family.
    siblings = sorted(
...
# --- orchestrator.py — appended Sub-2 module-level helpers (pre-flights + bipartite interleaving + affected_floor_set) ---
# =============================================================================
...
def make_per_floor_c9_runner(
    *,
    corridor_designed_candidates: tuple[Any, ...],
    grid: Any,
    plot_analysis: Any,
) -> Callable[[FloorRoomBrief], WetZonePlannedCandidate]:
    """Build a per-floor C9 + C10 runner for M8's cascade.

    Per Spec #4 v1.6 § 3.4 step 5: returns a callable that takes a
    `FloorRoomBrief` and returns a `WetZonePlannedCandidate` by
    running the real C9 + C10 pipeline. The corridor-designed
    candidates, grid, and plot_analysis are bound at factory time so
    the runner closure carries everything it needs.

    Args:
        corridor_designed_candidates: tuple from C8 (corridors) — the
            input batch to C9. The runner picks the FIRST element by
            default and runs C9 against it; multi-candidate fan-out
            is out-of-scope for M8 (which selects a single new-master
            cascade per affected floor).
        grid: C7 Grid carrying envelope dimensions.
        plot_analysis: C4 PlotAnalysis providing plot_facing.

    Returns:
        Callable that takes a `FloorRoomBrief` (with
        `has_master_bedroom` set per Spec #2 § 3.3) and returns a
        single `WetZonePlannedCandidate`.

    Raises (when called):
        Any C9 or C10 exception propagates. `apply_m8_real` catches
        them and routes to `invalidity_reason="c9_generation_failed"`
        (or `c10_validation_failed` once C10 failures become
        independently classifiable — currently both surface as
        c9_generation_failed at this layer; refinement is part of
        future telemetry work, not blocking the v1 contract).
    """
    if not corridor_designed_candidates:
        raise ValueError(
            "make_per_floor_c9_runner: corridor_designed_candidates "
            "must be non-empty."
        )

    def runner(per_floor_brief: FloorRoomBrief) -> WetZonePlannedCandidate:
        """Run C9 then C10 for the given per-floor brief.
        ...
        """
        # C9: room sizing against the per-floor brief.
...
################################################################################
# 4.10 tests/test_c11a/test_c11a_subsession1_schema.py — DELTA (6/6 partition sentinel)
################################################################################

# --- test_c11a_subsession1_schema.py — partition sentinel updated for new `generation` field ---
...
################################################################################
# 4.11 tests/test_c11a/test_c11a_subsession5_cache_versioning.py — DELTA (v1.3.0)
################################################################################

# --- test_c11a_subsession5_cache_versioning.py — version pin updated to v1.3.0 ---
