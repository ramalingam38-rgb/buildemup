...
| `RESEND_API_KEY` | _(required in prod)_ | Resend transactional email API key for the c3a CBA-checklist email (Component 3a / S7b § 7.2). Min length 10. |
| `RESEND_FROM` | `noreply@buildemup.com` | FROM address on the c3a CBA-checklist email. Must be domain-verified at Resend before production use. |
| `BUILDEMUP_DATABASE_PATH` | _(required in prod)_ | SQLite file path for the c3a gate_states + scheduler_state tables. In dev defaults via `_db_path()`; in prod must be explicit. |
| `BUILDEMUP_ADMIN_TOKEN` | _(required in prod)_ | Bearer token for /admin/scheduler/* endpoints. Min length 16. |
| `BUILDEMUP_PUBLIC_URL` | _(required in prod, dev default `http://localhost:8000`)_ | **NEW in S8 (§ 9.3 / round 2 X9).** Public-facing base URL. The c3a email hook uses this to build absolute cba-checklist URLs (e.g. `{base}/c3a/checklist.html?token=...&trace=...`). Forgetting to set this in prod causes startup failure (sys.exit(1)) — **NOT** silently broken email links. |
| `BUILDEMUP_GATE_DB_PATH` | unset | Override for the gate_states SQLite path (overrides `BUILDEMUP_DATABASE_PATH` for the c3a tables only). |
| `BUILDEMUP_ENV` | `dev` | Set to `prod` in production deploys. In `prod`, missing required env vars cause `sys.exit(1)` on startup (Railway treats as failed deploy). In `dev`, missing vars warn and continue. |
| `C3A_TEST_MODE` | unset | Test-only flag. When `1`, the c3aFetch wrapper in `_shared.js` disables the 503 auto-retry (auto-retry timing is itself a flake source in tests; External Item 3 / P36). Surfaced to the browser via `<meta name="c3a-test-mode" content="1">` injected by server.py on responses to `/c3a/*` requests when the env var is set. |
...
## Component 3a (Extreme-Case Gate) — UI + endpoints (NEW in S8)

**Per S8 SPEC v1.2 LOCKED.** Component 3a now ships a full UI surface
under `/c3a/*` plus two new endpoints (one read-only `/status`, one
GET-wrapper `/check-init`). Prior c3a sessions only exposed the four
state-mutating POST endpoints; S8 closes the user-visible gap.

### Routes added in S8

| Method | Path | Purpose |
|---|---|---|
| GET  | `/api/extreme-case/status` | Read-only state check; used by `done.html`/`aborted.html`/`checklist.html` for state reconciliation. Returns `is_terminal`, `terminal_state`, `last_updated_at`. |
| GET  | `/api/extreme-case/check-init` | POST-neutralizing wrapper around POST `/check`. Bot-User-Agent path returns minimal HTML preview (no session created). Browser path forwards to `handle_check`. Always carries `Cache-Control: no-store, no-cache, must-revalidate`. |
| GET  | `/c3a/case.html` | Initial entry — surfaces extreme case + options. |
| GET  | `/c3a/checklist.html` | CBA verification page — status-first then `cba-verified` POST. |
| GET  | `/c3a/done.html` | Terminal success view; reads `/status`, renders terminal-state-specific message. |
| GET  | `/c3a/aborted.html` | Abort confirmation; symmetric to `done.html` for `USER_ABORTED`. |
| GET  | `/c3a/_shared.css` + `_shared.js` | Design tokens + helpers (traceId, c3aFetch, displayError, handlePageFlow). |
| GET  | `/c3a/_test_harness.html` | Test-only page exposing handlePageFlow to window scope for the Phase-6 e2e suite (§ 4.11). **Route-guarded:** server.py returns 404 unless `C3A_TEST_MODE=1` is set in the environment, so production deploys never expose it even if the file ships in the asset bundle. |

### Required env var (NEW)

`BUILDEMUP_PUBLIC_URL` — see env-vars table above. Without this,
the cba-checklist email link is broken; in prod, server fails to
start. **Always include this in your Railway env config before the
first S8 deploy.**

### Smoke check after deploy

```bash
# 1. Both new endpoints exist
curl -s "$URL/api/extreme-case/status"
# → 400 with code=validation (no token supplied)
curl -s "$URL/api/extreme-case/check-init" -H "User-Agent: Mozilla/5.0"
# → 400 with code=validation (no brief supplied)

# 2. Bot-UA path returns HTML preview, no session created
curl -s "$URL/api/extreme-case/check-init?brief=t&request_id=r" \
     -H "User-Agent: facebookexternalhit/1.1" -i | head -20
# → HTTP/1.1 200; Content-Type: text/html; <!DOCTYPE html>...

# 3. Static UI pages load
curl -sI "$URL/c3a/case.html" | head -3
curl -sI "$URL/c3a/_shared.js" | head -3
# → both 200, correct Content-Type

# 4. Test harness is gated in production (C3A_TEST_MODE unset)
curl -s -o /dev/null -w "%{http_code}\n" "$URL/c3a/_test_harness.html"
# → 404 — the harness must NOT be reachable in prod
```

### Trace ID continuity (P43)

Server-side trace IDs (`server_trace_id`) are canonical for all
metrics, logs, and ops correlation. Client-side trace IDs
(`client_trace_id` — captured from `X-Trace-Id` header when valid
16-hex) surface in:
- the response body's `trace_id` field (correlation hint shown in UI)
- email links via the `&trace=...` query param (preserved across
  the 24h gap when the user clicks through)

If a customer reports an issue with a specific trace_id from the UI,
that trace lives in BOTH the server-side log lines (as
`client_trace_id=...`) and the response body — operators can grep
either.

### Operational notes

- **"Do not forward this email" UX hint** in the cba-checklist email
  is **expectation-setting, not security**. Real protection against
  email forwarding requires short-lived view tokens (B-042 backlog).
- **Referrer policy:** all C3a HTML pages set
  `<meta name="referrer" content="no-referrer">` to block ALL
  Referer-header leakage of session_token. (`no-referrer-when-downgrade`
  — the modern browser default — added no protection beyond what the
  browser already does.)
- **Test harness page** (`/c3a/_test_harness.html`) is gated at the
  route layer (server.py): with `C3A_TEST_MODE` unset or != `1`, the
  route returns 404 even though the file ships in the asset bundle.
  The e2e fixture in `tests/e2e/conftest.py` sets the flag for its
  lifetime; production deploys leave it unset. No build-time filter
  required.
- **Test-mode meta tag** — when `C3A_TEST_MODE=1`, server.py injects
  `<meta name="c3a-test-mode" content="1">` into `/c3a/*.html`
  responses (after `<head>`, idempotent). This surfaces the flag to
  `_shared.js isTestMode()` so `c3aFetch` disables its 503 auto-retry
  for deterministic test observation. Production responses are
  byte-identical to the on-disk files.
