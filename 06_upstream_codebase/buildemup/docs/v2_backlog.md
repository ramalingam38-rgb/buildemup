...
## v0.2 — Component 2 backlog (from Session D)

- **Room minimums branching:** requires Component 1 change. Currently
  RoomRequirement.__post_init__ rejects preferred_size below NBC minimum.
  To support Practical-vs-CodeStrict on room sizes, add a flag like
  `accept_below_nbc_minimum: bool = False` and route the validation through
  it. Then add `check_room_minimums_practical` + `_code_strict` + gap helper.

- **Cross-ventilation gap rarity:** v0.1 plot types (detached/semi-detached/
  continuous) all support ≥2 openable sides, so the ventilation gap rarely
  triggers. Adding interior-plot or shaft-only plot types would surface
  more meaningful gaps. For now, the check is built but mostly converges.

- **Solar exposure precision:** v0.1 scores 8 orientations on a 0-100 scale
  with arbitrary numbers (90 for S, 30 for N, etc). These are coarse
  approximations — should be re-grounded with city-specific solar geometry
  or accept user-provided neighbour-shading data.
