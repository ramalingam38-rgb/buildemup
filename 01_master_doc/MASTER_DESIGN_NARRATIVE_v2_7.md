...
| **Document version** | v2.2 — 29 April 2026 (this rebuild + Session 17 + C3a S1 shipped) |
| **Supersedes** | `BUILDEMUP_MASTER_DOCUMENT.md` v1.0 (28 April 2026, built from partial archive); v2.0 + v2.1 (earlier on 29 April) |
...
### ⏳ Component 3a — Extreme Case Gate — Sessions 1–5 of 8 SHIPPED (in build directory)
...
**Status:** Active build. Parent SPEC v0.2.1a LOCKED. S4 SPEC v0.1 LOCKED + shipped. S5 SPEC v1.0 LOCKED + shipped (incl. critique-round-2 fixes v1.1). **B-027 (S6 prerequisite) v1.0 LOCKED + shipped** (incl. code-critique fixes v1.1). **S6 SPEC v1.0 LOCKED, ready for next-conversation code build.** Sessions 1–5 of 8 shipped + B-027 shipped; **165 tests passing across 8 suites**.

**Deliverables (S1–S4):** see prior session entries (4.18–4.20).

**Deliverables (S5):**
- `components/c03a/counterfactual.py` — 180 LOC, public `build_counterfactual(decision)` + `_format_would_have` with P1 acronym preservation + D1 contract assertion
- `components/c03a/preflight.py` — 345 LOC, public `build_preflight_summary(cases)` + P2 hint-with-reasons + P3 severity-aware messaging + D4 single-pass refactor + D2 module-level string constants
- `tests/test_c03a_session5_counterfactual.py` — 323 LOC, 12 tests
- `tests/test_c03a_session5_preflight.py` — 389 LOC, 17 tests
- `tests/test_c03a_session5_detector_contract.py` — 300 LOC, 8 tests (P4 — locks the S2↔S5 contract loudly)
- Full c3a suite **158 / 158 PASS**

**Sessions remaining (S6–S8):**
- **S6** — Orchestrator (~450 LOC, ~12 tests) — `components/c03a_extreme_case_gate.py`. Spec drafting begins per D-066 cycle.
- **S7** — API endpoints (~290 LOC, ~6 tests)
- **S8** — Validation suite (~700 LOC, ~7 tests)

**Total estimated remaining:** ~1,440 LOC source after S5. Calendar: 3 conversations at S2/S3/S4/S5 cadence (or fewer with D-066 multi-session conversations).

**Backlog:** 38 items in `v0_2_backlog.md` (B-001 through B-038). B-016 through B-025 from S5 critique. B-026 superseded into B-023. B-027 was a prerequisite shipped this session, not backlog. B-028 through B-034 from S6 critique rounds. B-035 through B-038 from B-027 code critique deferrals.
...
## 11.1 Starting point — Session 22 (fresh conversation)

**Next session:** Component 3a Session 6 — Orchestrator (`c03a_extreme_case_gate.py`).

**S6 SPEC v1.0 is LOCKED.** The handoff package contains `buildemup_S6_SPEC_v1_0_LOCKED.txt` ready for the next Claude to read and code from. No further spec critique round needed; spec went through 3 rounds and the third was clean.

The Claude that resumes reads (in order):
1. `00_START_HERE/NEXT_CLAUDE_HANDOFF.md` — top-level orientation, build cycle rule, three obligations, five patterns
2. `01_master_doc/MASTER_DESIGN_NARRATIVE.md` (currently v2.7) — esp. Parts 4.18–4.22, 8, 9 (D-064/D-065/D-066/D-067), 11
3. `02_specs_chronological/buildemup_S6_SPEC_v1_0_LOCKED.txt` — what to build
4. Parent C3a spec for context: § 4.1–4.5 (orchestrator flow), § 4.8 (error handling), § 6 (loop termination), § 9.5 (S6 test plan)
5. `03_code_chronological/` — all shipped C3a deliverables S1 through S5 + B-027

## 11.2 The S6 build plan

Per locked S6 v1.0 spec:

S6's `c03a_extreme_case_gate.py` is the orchestrator that wires:
- **Detection** (S2): runs at iteration 0 + after every successful apply
- **Preflight** (S5): built once at iter 0 via `build_preflight_summary(cases)`
- **Per-iteration loop**: present current EC → user picks option → apply via S4 → re-run C2 → re-detect; terminate on no-blockers / Preview / abort / max-iter / per-case-limit / CBA pause
- **State machine**: `class ExtremeCaseGate` with `start()` + `apply_user_decision()` + `apply_user_abort()`. `GateState` is immutable, returned by every method.
- **Typed UI events** (per spec P1): `TransitionBannerEvent` + `DifferentPlotPromotionEvent` instead of free-form strings
- **failed_option_ids_for_current_case** (per spec P3): `frozenset[str]` — backend rejects re-selection
- **feasibility_input plumbed through every C2 call** (per spec P4 — real bug fix from v0.1)
- **user_acknowledged_at caller-supplied** (per spec P5): no `_iso_now()` in S6
- **Sticky different-plot promotion** (per spec P6): once promoted, applies to all subsequent cases
- **Counterfactuals** (S5): built only on SUCCESS / PREVIEW_MODE (not abort paths)

**B-027 is shipped:** S1 has `is_different_plot_option`; S3 uses `_build_different_plot_option`; S5 has public `classify_case_severity`. S6 imports the public name.

Per D-066: S6 will follow spec → already locked → code → critique → patch → next (S7).

## 11.3 Sessions remaining for C3a (S6–S8)

| Session | Deliverable | Estimated LOC | Tests |
|---|---|---|---|
| S6 | Orchestrator (`c03a_extreme_case_gate.py`) — locked v1.0 spec ~790 LOC source | ~790 | ~19 |
| S7 | API endpoints — 5 endpoints incl. /cba-fallback-continue, email-trigger for CBA, 24h scheduler hook | ~290 | ~6 |
| S8 | Validation suite — integration tests across 10 EC paths + Preview + CBA fallback + preflight + reason-aware errors | ~700 | ~7 |

**Total remaining after B-027:** ~1,780 LOC source across 3 sessions.

## 11.4 After C3a fully ships (S8 complete)

Per master doc Update Protocol Part 13:
1. Update Part 4 with Sessions 22–24 (S6 through S8)
2. Update Part 8 (Build Status: 4 of 17 components built, 13 to go)
3. Update Part 9 with decisions D-068+
4. Update Part 12 with all C3a code embedded
...
- ❌ Don't write code before consulting the LOCKED spec (Obligation 1 + D-066)
- ❌ Don't draft a new spec for S6 — it's already locked at v1.0
- ❌ Don't skip the CODE critique round after S6 build completes (D-066 mandates it)
- ❌ Don't modify S1/S2/S3/S4/S5/B-027 deliverables — they're locked by their tests
- ❌ Don't fall into Pattern A/B/C/D/E
- ❌ Don't refer to "Component 3" unqualified — always 3a or 3b
- ❌ Don't suggest handoff (per D-066 v2). Continue building until USER says stop. ONE EXCEPTION: if context is genuinely too low to complete current step at quality level of prior chats, tell user straight as a quality-protection signal.
- ❌ Don't accept every critique round verbatim — apply D-067 pushback rule when critique would force YAGNI surgery / hostile mechanisms / fixes for hypothetical futures.
...
**Version:** v2.7 — 30 April 2026 (Session 18 + C3a S1, S2, S3, S4, S5 + B-027 shipped — 5 of 8 sessions complete; D-064/D-065/D-066/D-067/D-068/D-069 build-cycle + handoff rules locked; S6 v1.0 LOCKED ready for next-conversation code build)
